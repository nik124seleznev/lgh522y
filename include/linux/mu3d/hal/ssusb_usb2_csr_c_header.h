...
//                  
#define GO_POLLING                                (0x1<<7) //   

//                    
#define LPM_BESL_STALL                            (0x1<<14) //     
#define LPM_BESLD_STALL                           (0x1<<13) //     
#define BC12_EN                                   (0x1<<12) //     
#define LPM_RWP                                   (0x1<<11) //     
#define LPM_HRWE                                  (0x1<<10) //     
#define LPM_MODE                                  (0x3<<8) //   
#define ISO_UPDATE                                (0x1<<7) //   
#define SOFT_CONN                                 (0x1<<6) //   
#define HS_ENABLE                                 (0x1<<5) //   
#define HS_MODE                                   (0x1<<4) //   
#define BUS_RESET                                 (0x1<<3) //   
#define RESUME                                    (0x1<<2) //   
#define SUSPEND                                   (0x1<<1) //   
#define SUSPENDM_ENABLE                           (0x1<<0) //   

//                    
#define FS_DIS_SEL                                (0xf<<4) //   
#define PHY_CLK_VALID                             (0x1<<3) //   
#define FS_DIS_NE                                 (0x1<<2) //   
#define TM1                                       (0x1<<0) //   

//                  
#define HW_AUTO_SENDRST_EN                        (0x1<<8) //   
#define B_DEV                                     (0x1<<7) //   
#define FS_DEV                                    (0x1<<6) //   
#define LS_DEV                                    (0x1<<5) //   
#define VBUS                                      (0x3<<3) //   
#define HOSTMODE                                  (0x1<<2) //   
#define HOSTREQ                                   (0x1<<1) //   
#define SESSION                                   (0x1<<0) //   

//                    
#define LPM_HUBDRVRMP_TIME                        (0xffff<<8) //    
#define PWR_UP_CNT_LMT                            (0xf<<0) //   

//                  
#define U2U3_AUTO_SWITCH                          (0x1<<10) //     
#define HOST_FORCE_EN                             (0x1<<9) //   
#define LPM_FORCE_STALL                           (0x1<<8) //   
#define FORCE_HOST                                (0x1<<7) //   
#define FIFO_ACCESS                               (0x1<<6) //   
#define FORCE_FS                                  (0x1<<5) //   
#define FORCE_HS                                  (0x1<<4) //   
#define TEST_PACKET_MODE                          (0x1<<3) //   
#define TEST_K_MODE                               (0x1<<2) //   
#define TEST_J_MODE                               (0x1<<1) //   
#define TEST_SE0_NAK_MODE                         (0x1<<0) //   

//                          
#define LPM_RESUME_INTR_EN                        (0x1<<9) //   
#define LPM_INTR_EN                               (0x1<<8) //   
#define VBUSERR_INTR_EN                           (0x1<<7) //   
#define SESSION_REQ_INTR_EN                       (0x1<<6) //   
#define DISCONN_INTR_EN                           (0x1<<5) //   
#define CONN_INTR_EN                              (0x1<<4) //   
#define SOF_INTR_EN                               (0x1<<3) //   
#define RESET_INTR_EN                             (0x1<<2) //   
#define RESUME_INTR_EN                            (0x1<<1) //   
#define SUSPEND_INTR_EN                           (0x1<<0) //   

//                   
#define LPM_RESUME_INTR                           (0x1<<9) //   
#define LPM_INTR                                  (0x1<<8) //   
#define VBUSERR_INTR                              (0x1<<7) //   
#define SESSION_REQ_INTR                          (0x1<<6) //   
#define DISCONN_INTR                              (0x1<<5) //   
#define CONN_INTR                                 (0x1<<4) //   
#define SOF_INTR                                  (0x1<<3) //   
#define RESET_INTR                                (0x1<<2) //   
#define RESUME_INTR                               (0x1<<1) //   
#define SUSPEND_INTR                              (0x1<<0) //   

//                       
#define XFER_START_FROM_SOF                       (0x1<<24) //     
#define VBUSERR_MODE                              (0x1<<23) //     
#define TX_FLUSH_EN                               (0x1<<22) //     
#define NOISE_STILL_SOF                           (0x1<<21) //     
#define UNDO_SRP_FIX                              (0x1<<19) //     
#define OTG_DEGLITCH_DISABLE                      (0x1<<18) //     
#define SWRST                                     (0x1<<17) //     
#define DIS_USB_RESET                             (0x1<<16) //     
#define SOFT_DEBOUCE                              (0x1<<0) //   

//                   
#define WTWRSM                                    (0xf<<28) //     
#define WTRSMK                                    (0xf<<24) //     
#define WRFSSE0                                   (0xf<<20) //     
#define WTCHRP                                    (0xf<<16) //     
#define VPLEN                                     (0xff<<8) //    
#define WTCON                                     (0xf<<4) //   
#define WTID                                      (0xf<<0) //   

//                           
#define USB20_RESET_TIME_VALUE                    (0xffff<<0) //    
...
//                   
#define FRAME_NUMBER                              (0x7ff<<0) //    

//                          
#define CHOPPER_DELAY_TIME                        (0xff<<16) //     
#define SOFTCON_DELAY_TIME                        (0xff<<8) //    
#define TIME_VALUE_1US                            (0xff<<0) //   

//                       
#define BESLCK_U3                                 (0xf<<12) //     
#define BESLCK                                    (0xf<<8) //    
#define BESLDCK                                   (0xf<<4) //   
#define BESL                                      (0xf<<0) //   

//                         
#define LPM_EXIT_COUNT                            (0xff<<16) //     
#define LPM_EXIT_COUNT_RESET                      (0x1<<9) //   
#define LPM_ENTRY_COUNT_RESET                     (0x1<<8) //   
#define LPM_ENTRY_COUNT                           (0xff<<0) //   

//                      
#define LPM_U3_ACK_EN                             (0x1<<0) //   

//                          
#define LPM_L1_TOKENRETRY                         (0x1ff<<16) //     
#define LPM_L1_RESIDENCY                          (0xfff<<0) //    

//                 
#define OPSTATE_SYS                               (0x3f<<0) //   
