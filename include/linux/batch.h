...
#define BATCH_VER(fmt, args...)   printk(BATCH_TAG"%s: "fmt, __func__, ##args) //         
...
{
	int (*enable_hw_batch)(int handle, int enable, long long samplingPeriodNs,long long maxBatchReportLatencyNs);//                                                         
...
{
	int (*get_data)(int handle, hwm_sensor_data *data);//                                                                                                           
...
{
	struct batch_control_path 	ctl_dev[MAX_ANDROID_SENSOR_NUM+1];//                                                                                                                         
	struct batch_data_path 		data_dev[MAX_ANDROID_SENSOR_NUM+1];//                                                                                                                                    
};
