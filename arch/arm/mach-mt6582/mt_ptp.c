...
unsigned int PTP_INIT_01(void)
{
    ...
    ptp_init_value.VMAX        = 0x5D;    //                              
    #ifdef CONFIG_MTK_SDIOAUTOK_SUPPORT
    if (is_vcore_ss_corner())
    {
        ptp_init_value.VMIN    = 0x4E;    //                             
    }
    else
    {
        ptp_init_value.VMIN    = 0x48;    //                             
    }
    #else
    ptp_init_value.VMIN        = 0x48;    //                             
    #endif
    ptp_init_value.DTHI        = 0x01;    //         
    ptp_init_value.DTLO        = 0xfe;    //                         
    ptp_init_value.VBOOT       = 0x48;    //                           
    ptp_init_value.DETMAX      = 0xffff;  //                                            

    if (ptp_version == 1)
        ptp_init_value.PTPCHKSHIFT = 0x67; //                   
...
irqreturn_t mt_ptp_isr(int irq, void *dev_id)
{
    ...
    if (PTPINTSTS == 0x1) //                   
    {
        if ((temp_ptpen & 0x7) == 0x1) //          
        {
            ...
            mt_cpufreq_enable_by_ptpod(); //            
            mt_fh_popod_restore(); //                                    

            PTP_INIT_02();
        }
        else if ((temp_ptpen & 0x7) == 0x5) //          
        {
...
unsigned int PTP_INIT_01_API(void)
{
    ...
    //                         
    freq_0 = (u8)(mt_cpufreq_max_frequency_by_DVS(0) / 12000); //                     
    freq_1 = (u8)(mt_cpufreq_max_frequency_by_DVS(1) / 12000); //     
    freq_2 = (u8)(mt_cpufreq_max_frequency_by_DVS(2) / 12000); //    
    freq_3 = (u8)(mt_cpufreq_max_frequency_by_DVS(3) / 12000); //    
...
static int ptp_probe(struct platform_device *pdev)
{
    ...
    //                         
    freq_0 = (unsigned char)(mt_cpufreq_max_frequency_by_DVS(0) / 12000);   //                     
