...
void dcm_enable(unsigned int type)
{
	...
	if (type & TOPCKGEN_DCM) {
        ...
        //                                         
        dcm_writel(DCM_CFG, 0xFFFFFF7F);//                                               
        #endif
        dcm_setl(CLK_SCP_CFG_0, 0x3FF);//                                      
        dcm_setl(CLK_SCP_CFG_1, ((0x1 << 4) | 0x1));//                                            
    ...
    if (type & IFR_DCM) {
		...
		dcm_clrl(TOP_CKDIV1, 0x0000001f);//               
		dcm_setl(TOP_DCMCTL, 0x00000007);//               
		dcm_setl(TOP_DCMDBC, 0x00000001);//                        
        dcm_setl(INFRA_DCMCTL, 0x00000303);//                                                  
        dcm_setl(INFRA_DCMDBC, 0x00000300);//                     
        dcm_clrl(INFRA_DCMDBC, 0x0000007F);//                


		#if 0//                          
		dcm_writel(INFRA_DCMFSEL, 0xFFE0F0F8);//                                                  
        #else//               
		dcm_writel(INFRA_DCMFSEL, 0xFFF0F0F8);//                                            
        #endif
    ...
    if (type & PER_DCM) {
        ...
        dcm_setl(PERI_GLOBALCON_DCMDBC, 0x1<<7);//          
		dcm_clrl(PERI_GLOBALCON_DCMDBC, 0x0000007F);//                 

		dcm_clrl(PERI_GLOBALCON_DCMFSEL,0x00000007);//               
        dcm_clrl(PERI_GLOBALCON_DCMFSEL,0x00000F00);//                
        dcm_clrl(PERI_GLOBALCON_DCMFSEL,0x001F0000);//                 

		//                                   
		//           
		dcm_clrl(MSDC0_IP_DCM, 0xFF800000);//                   
		dcm_clrl(MSDC1_IP_DCM, 0xFF800000);//                   
		dcm_clrl(MSDC2_IP_DCM, 0xFF800000);//                   


		//   
		dcm_clrl(PERI_USB0_DCM, 0x00070000);//                   

        //    
        dcm_setl(PMIC_WRAP_DCM_EN, 0x1);//          

		//   
		dcm_setl(I2C0_I2CREG_HW_CG_EN, 0x1);//          
		dcm_setl(I2C1_I2CREG_HW_CG_EN, 0x1);//          
		dcm_setl(I2C2_I2CREG_HW_CG_EN, 0x1);//          
    ...
    if (type & SMI_DCM) {
        ...
        //          
        dcm_writel(SMI_DCM_CONTROL, 0x1);//           
		//  
    ...
    if (type & DIS_DCM) {
        ...
        if (subsys_is_on(SYS_DIS)) {
	        ...
	        dcm_setl(SMILARB0_DCM_SET, 0x1<<15);//           
			//   
			dcm_readl(SMILARB0_DCM_CON);
    ...
    if (type & ISP_DCM) {
        ...
        if (subsys_is_on(SYS_ISP)) {
            dcm_writel(CAM_CTL_RAW_DCM, 0xFFFF8000);//                
            dcm_writel(CAM_CTL_RGB_DCM, 0xFFFFFE00);//               
            dcm_writel(CAM_CTL_YUV_DCM, 0xFFFFFFF0);//               
            dcm_writel(CAM_CTL_CDP_DCM, 0xFFFFFE00);//               
            dcm_writel(CAM_CTL_DMA_DCM, 0xFFFFFFC0);//               

            dcm_clrl(JPGENC_DCM_CTRL, 0x1);//            


            //                                         
            dcm_setl(VENC_CLK_DCM_CTRL, 0x1);//  
	        ...
	        dcm_setl(SMILARB2_DCM_SET, 0x1<<15);//           
			//   
			dcm_readl(SMILARB2_DCM_CON);
    ...
    if (type & VDE_DCM) {
        ...
        if (subsys_is_on(SYS_VDE)) {
	        ...
	        dcm_setl(SMILARB1_DCM_SET, 0x1<<15);//           
			//   
...
//                            
    ...
    if (type & CPU_DCM) {
        ...
        dcm_info("[%s][CPU_DCM     ]=0x%08x\n", __func__,CPU_DCM);
        dcm_clrl(MCU_BIU_CON, 0x1 << 12);//           
        dcm_clrl(CA7_MISC_CONFIG, 0x1 << 9);//          
        dcm_sta &= ~CPU_DCM;
    }
    ...
    if (type & PER_DCM) {
        ...
        dcm_clrl(PERI_GLOBALCON_DCMCTL, 0x00001F00);//                  
        dcm_clrl(PERI_GLOBALCON_DCMCTL, 0x000000F3);//                         
		...
		//                                   
		//           
		dcm_setl(MSDC0_IP_DCM, 0xFF800000);//                 
		dcm_setl(MSDC1_IP_DCM, 0xFF800000);//                 
		dcm_setl(MSDC2_IP_DCM, 0xFF800000);//                 


		//   
		dcm_setl(PERI_USB0_DCM, 0x00070000);//                 

        //    
        dcm_clrl(PMIC_WRAP_DCM_EN, 0x1);//          

		//   
		dcm_clrl(I2C0_I2CREG_HW_CG_EN, 0x1);//          
		dcm_clrl(I2C1_I2CREG_HW_CG_EN, 0x1);//          
		dcm_clrl(I2C2_I2CREG_HW_CG_EN, 0x1);//          
    ...
    if (type & IFR_DCM) {
        ...
        dcm_info("[%s][IFR_DCM     ]=0x%08x\n", __func__,IFR_DCM);
		/*                                          */
		dcm_setl(DRAMC_PD_CTRL, 0x1 << 24);//           
		dcm_clrl(DRAMC_PD_CTRL, 0x1 << 25);//           

        //          
		//                                                     

        dcm_clrl(TOP_DCMCTL, 0x00000006);//                                            

		//                                  
        dcm_clrl(INFRA_DCMCTL, 0x00000303);//                                                  
        //          
        //                                                          
        //                                                     
    ...
    if (type & SMI_DCM) {
        ...
        //          
        dcm_clrl(SMI_DCM_CONTROL, 0x1);//          

		//              
        dcm_readl(SMI_COMMON_AO_SMI_CON);
		//              
        dcm_readl(SMI_COMMON_AO_SMI_CON_SET);
	    dcm_setl(SMI_COMMON_AO_SMI_CON_CLR,0x4);//          

        //       
        dcm_clrl(MMU_DCM, 0x1);//          
    ...
    if (type & DIS_DCM) {
		...
		dcm_setl(SMILARB0_DCM_CLR,(0x1<<15));//           
    ...
    if (type & ISP_DCM) {
        ...
        dcm_setl(CAM_CTL_RAW_DCM, 0x00007FFF);//                
        dcm_setl(CAM_CTL_RGB_DCM, 0x000001FF);//               
        dcm_setl(CAM_CTL_YUV_DCM, 0x0000000F);//               
        dcm_setl(CAM_CTL_CDP_DCM, 0x000001FF);//               
        dcm_setl(CAM_CTL_DMA_DCM, 0x0000003F);//               

        dcm_setl(JPGENC_DCM_CTRL, 0x00000001);//          


		//                                         
        dcm_writel(VENC_CLK_DCM_CTRL, 0xFFFFFFFE);//          
		...
		dcm_setl(SMILARB2_DCM_CLR,(0x1<<15));//           
    ...
    if (type & VDE_DCM) {
        ...
        //   
        dcm_readl(SMILARB1_DCM_SET);
		dcm_setl(SMILARB1_DCM_CLR,(0x1<<15));//           
