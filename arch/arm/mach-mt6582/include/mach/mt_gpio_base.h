...
typedef struct {
    VAL_REGS    dir[11];            /*                          */
    unsigned char       rsv00[80];  /*                          */
    VAL_REGS    pullen[11];         /*                          */
    unsigned char       rsv01[80];  /*                          */
    VAL_REGS    pullsel[11];        /*                          */
    unsigned char       rsv02[80];  /*                          */
    unsigned char       rsv03[256]; /*                          */
    VAL_REGS    dout[11];           /*                          */
    unsigned char       rsv04[80];  /*                          */
    VAL_REGS    din[11];            /*                          */
    unsigned char       rsv05[80];	/*                          */
    VAL_REGS    mode[36];           /*                          */  
	unsigned char		rsv06[192];	/*                          */
	VAL_REGS    ies[2];            	/*                          */
    VAL_REGS    smt[2];        		/*                          */ 
	unsigned char		rsv07[192];	/*                          */
	VAL_REGS    tdsel[8];        	/*                          */ 
	VAL_REGS    rdsel[8];        	/*                          */ 
	VAL_REGS    drv_mode[8];        /*                          */ 
	unsigned char		rsv08[128];	/*                          */
	VAL_REGS    msdc0_ctrl[4];      /*                          */ 
	VAL_REGS    msdc1_ctrl[4];      /*                          */ 
	VAL_REGS    msdc2_ctrl[4];      /*                          */
	VAL_REGS    exmd_ctrl[1];       /*                          */
	VAL_REGS    bpi_ctrl[1];       	/*                          */
	VAL_REGS    kpad_ctrl[2];       /*                          */
} GPIO_REGS;
