...
void eemcs_memory_reserve(void){
	...
	if( (ext_md_usage_case&MD5_EN)== MD5_EN) { //                
		md5_en = 1;
	...
	switch(active_id) {
		case MD_SYS5:
			if (md_setting_flag == MD_2G_FLAG){
				ext_md_support[active_index] = modem_2g;
			} else if (md_setting_flag == MD_WG_FLAG){
				ext_md_support[active_index] = modem_wg;
			} else if (md_setting_flag == MD_TG_FLAG){
				ext_md_support[active_index] = modem_tg;
