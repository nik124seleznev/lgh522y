...
static ssize_t store_ignore_nice_load(struct dbs_data *dbs_data, const char *buf, size_t count)
{
	...
	if (input == hp_tuners->ignore_nice_load)	/*               */
		return count;
			    ...
			    unsigned int code, int value)
{
	...
	if (per_cpu(hp_cpu_dbs_info, 0).cdbs.cur_policy == NULL)
		return;
	dbs_data = per_cpu(hp_cpu_dbs_info, 0).cdbs.cur_policy->governor_data;	/*                      */
#endif
	if (!dbs_data)
		return;
	hp_tuners = dbs_data->tuners;
