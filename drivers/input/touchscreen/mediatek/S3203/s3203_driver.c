...
static int tpd_power(struct i2c_client *client, int on)
{
	...
	if (on)
    {       
        ret = tpd_i2c_write_byte_data(client, fd_01.controlBase, 0x80);/*         */	
		if (ret < 0)
        {
        ...
        ret = tpd_i2c_write_byte_data(client, fd_01.commandBase, 0x01);/*                 */
        if (ret < 0)
        {
	...
	for(i = 0 ; i < 4; i++)
	{
		ret = tpd_i2c_write_byte_data(client, (fd_11.controlBase + 6 + i), tpd_res[i]); /*                               */
		if (ret < 0)
        	{
    ...
    {
		ret = tpd_i2c_write_byte_data(client, fd_01.controlBase, 0x81); /*                                   */
		if (ret < 0)
        {
