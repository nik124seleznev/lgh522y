...
static error_type lpwg_control(struct synaptics_ts_data *ts)
{
	...
	switch (ts->reportMode)
	{
		...
		case T_REPORT_KNOCK_ON_CODE:					//                
			...
			tci_control(ts, INTERRUPT_DELAY_CTRL, (u8)ts->lpwgSetting.isFirstTwoTapSame);	//                      

			tci_control(ts, TCI_ENABLE_CTRL2, 1);		//             
			tci_control(ts, TAP_COUNT_CTRL2, (u8)ts->lpwgSetting.tapCount); //                           
			...
			tci_control(ts, INTERRUPT_DELAY_CTRL2, 1);	//                      
