...
#define  NVRAM_LEN               0x0FF0   //                  
#define  NVRAM_BOOT_SECTOR_LEN	 0x0100	//             
#define  NVRAM_UPDATE_START_ADDR 0x4100
#define  I2C_FRAME_MAX_LENGTH    6   //                 

#define  guitar_i2c_address      0xBA

#define GT818_SET_INT_PIN( level ) mt_set_gpio_out(GPIO_CTP_EINT_PIN, level) //              
...
int __init gt818_downloader( struct i2c_client *client, unsigned short ver, unsigned char * data )
{
	...
	if( ((ver&0xff) > (fw_info->version&0xff)) )   //                                              
		{
