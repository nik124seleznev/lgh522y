...
//       
#define TWO_D_REPORT_MODE_REG		(ts->finger_fc.dsc.control_base)//       
...
#define LPWG_TAPCOUNT_REG					0x47 //       
#define LPWG_MIN_INTERTAP_REG				0x48 //       
#define LPWG_MAX_INTERTAP_REG				0x49 //       
#define LPWG_TOUCH_SLOP_REG					0x4A //       
#define LPWG_TAP_DISTANCE_REG				0x4B //       
...
//  
	...
	switch(newState)
	{
		...
		case STATE_KNOCK_ON_CODE:		//                
			...
			sleep_mode_set(client);
			tci_control(client, MAX_INTERTAP_CTRL, 70);		//                      
			tci_control(client, TAP_DISTANCE_CTRL, 255);		//                   
			tci_control(client, TAP_COUNT_CTRL, (u8)ts->lpwgSetting.tapCount); //                           
			tci_control(client, REPORT_MODE_CTRL, 1);		//                    
			break;
		case STATE_OFF:
...
static int S7020_set_qcover_area(struct i2c_client *client,u32 X1, u32 X2, u32 Y1, u32 Y2)
{
	...
	buffer[1] = value[0];//      
	buffer[2] = value[2];//      
	buffer[3] = (value[2] & 0xF00) | (value[0]>>8);//                
	buffer[4] = value[1];
	buffer[5] = value[3];
...
static int S7020_InterruptHandler(struct i2c_client *client,TouchReadData *pData)
{
	...
	//                                          
		...
		if (1)/*                  */ {
			...
			for (i = 0; i < MAX_NUM_OF_FINGERS; i++) {
				...
				if (( ( sensor_data.finger_state[reg_num] >> ( finger_order * 2 ) ) & 0x03 ) == 1) {
					...
					pFingerData->id = i;
					pFingerData->type = ( sensor_data.finger_state[reg_num] >> ( finger_order * 2 ) );//                                                                 
					...
					//                                                                                                                     
					//                                                                          
					pFingerData->pressure = sensor_data.finger_data[i][PRESSURE];//       
		...
		if ((status & 0x1) && (ts->currState == STATE_KNOCK_ON_ONLY)) { /*                    */
			TOUCH_LOG( "Knock-on Detected\n" );
			pData->type = DATA_KNOCK_ON;
		} else if ((status & 0x1) && (ts->currState == STATE_KNOCK_ON_CODE)) {  /*                   */
