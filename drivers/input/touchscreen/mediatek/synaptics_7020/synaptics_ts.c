...
#define SMALL_OBJECT_DETECTION_TUNNING_REG	(ts->finger_dsc.control_base+45) //      
...
#define FLASH_CONFIG_ID_REG				(ts->flash_dsc.control_base)		/*               */
...
static void synaptics_ts_suspend_func(struct synaptics_ts_data *ts)
{
	...
	ret = i2c_smbus_write_byte_data(ts->client, DEVICE_CONTROL_REG, DEVICE_CONTROL_REG_SLEEP); /*       */
