...
static u8 tpd_get_ps_value(void)
{
	...
	if (tpd_proximity_flag == 1 && obj_ps.polling == 1)  //             
    {
...
//                                                                                                         
    ...
    if (tpd_proximity_flag == 1 && obj_ps.polling == 0) //               
    {
        //                                                
		i2c_smbus_read_i2c_block_data(i2c_client, 0x01, 1, &proximity_status);
        printk("REG INDEX[0x01]:0x%02X\n", proximity_status);
...
//                                 
...
#if 0//                                                                    
		cinfo->x[i] = cinfo->x[i]*540/480;//                         
#endif
#if 0//                                                                     
		cinfo->y[i] = cinfo->y[i]*960/854;//                          
#endif
	...
	for(i = 0; i < point_num; i++)
	{
		cinfo->x[i] = cinfo->x[i] *36/51;//           
		cinfo->y[i] = cinfo->y[i] *27/37;//           
	}
#endif
...
{
    unsigned char uc_i2C_addr;             //                                 
    unsigned char uc_io_voltage;           //                              
    unsigned char uc_panel_factory_id;     //                   
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
	...
	udelay(200);
		...
		upgrate_percent = (j* 100)/packet_number ; //       
		//                                                                          
    }
