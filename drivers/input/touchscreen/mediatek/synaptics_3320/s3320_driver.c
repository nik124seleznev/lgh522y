...
static int lpwg_control ( int mode )
{
	...
	switch ( mode )
	{
		case LPWG_DOUBLE_TAP:	//                
			...
			tci_control ( tpd_i2c_client, MIN_INTERTAP_CTRL, 0 );		//                  
			tci_control ( tpd_i2c_client, MAX_INTERTAP_CTRL, 70 );	 	//                      
			tci_control ( tpd_i2c_client, TOUCH_SLOP_CTRL, 100 );		//                  
			tci_control ( tpd_i2c_client, TAP_DISTANCE_CTRL, 10 );	 	//                    
			tci_control ( tpd_i2c_client, INTERRUPT_DELAY_CTRL, 0 );	//                      
			tci_control ( tpd_i2c_client, TCI_ENABLE_CTRL2, 0 );		//              
		...
		case LPWG_PASSWORD:		//                         
			...
			tci_control ( tpd_i2c_client, MAX_INTERTAP_CTRL, 70 );		//                      
			tci_control ( tpd_i2c_client, TOUCH_SLOP_CTRL, 100 );		//                  
			tci_control ( tpd_i2c_client, TAP_DISTANCE_CTRL, 7 );		//                   
			tci_control ( tpd_i2c_client, INTERRUPT_DELAY_CTRL, double_tap_check );	//                      

			tci_control ( tpd_i2c_client, TCI_ENABLE_CTRL2, 1 );		//             
			tci_control ( tpd_i2c_client, TAP_COUNT_CTRL2, lpwg_tap_count ); //                           
			tci_control ( tpd_i2c_client, MIN_INTERTAP_CTRL2, 0 );		//                  
			tci_control ( tpd_i2c_client, MAX_INTERTAP_CTRL2, 70 );		//                      
			tci_control ( tpd_i2c_client, TOUCH_SLOP_CTRL2, 100 );		//                  
			tci_control ( tpd_i2c_client, TAP_DISTANCE_CTRL2, 255 );	//                   
			tci_control ( tpd_i2c_client, INTERRUPT_DELAY_CTRL2, 0 );	//                      
...
static int lpwg_update_all ( struct work_struct *lpwg_update_timer )
{
	...
	else if ( !lpwg_ctrl.screen && lpwg_ctrl.qcover )	//                  
	{
		sleep_status = 1;
		lpwg_status = 1;
	}
	else if ( !lpwg_ctrl.screen && !lpwg_ctrl.qcover && lpwg_ctrl.sensor )	//                           
	{
		sleep_status = 1;
...
static int synaptics_touch_event_handler ( void *unused )
{
	...
	do
	{
		...
		/*                        */
		if ( int_status & INTERRUPT_MASK_LPWG )	//          
		{
		...
		else if ( int_status & INTERRUPT_MASK_ABS0 )	//            
		{
