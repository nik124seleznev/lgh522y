...
#define CV_CHECK_DELAT_FOR_BANDGAP	80		/*      */
...
kal_uint32 		charging_full_current = CHARGING_FULL_CURRENT;	/*    */
...
{
 	kal_bool ta_current_pattern = KAL_FALSE;		//                 
...
static void mtk_ta_detector(void)
{
	...
	do {
		...
		if(real_v_chrA - real_v_chrB >= 300) {	/*      */
...
static void mtk_ta_vchr_select(int i,int ta_v_chr_candidate[], int ta_charging_current_candidate[], int *max_charging_current, int *max_charging_current_i)
{
	...
	battery_charging_control(CHARGING_CMD_SET_CURRENT,&ta_charging_current);	//    
...
static void battery_pump_express_algorithm_start(void)
{
	...
	if(is_ta_connect == KAL_TRUE) {
		...
		curr_vbat = battery_meter_get_battery_voltage();
		if( ((curr_vbat-ta_pre_vbat)>100) && (curr_vbat < (cv_voltage - (CV_CHECK_DELAT_FOR_BANDGAP+20))) && (curr_vbat > TA_START_VCHR_TUNUNG_VOLTAGE) ) {		/*                                                         */
		...
		/*                       */
		if((is_ta_connect == KAL_TRUE) && (curr_vbat > TA_START_VCHR_TUNUNG_VOLTAGE) &&(ta_check_ta_control == KAL_TRUE)) {	/*                                             */
		 	battery_charging_control(CHARGING_CMD_SET_CURRENT,&ta_charging_current);	/*      */
