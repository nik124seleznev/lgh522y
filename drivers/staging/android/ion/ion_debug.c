...
{
	return get_string(string_name,len+1,&gKernelPathTable); //              
}
//                                                                
	...
	if(new_node != NULL)
	{
		 struct ion_record_basic_info *tracking_info = NULL;
	         tmp = record_param->client;
            	...
            	tracking_info->recordID.client = record_param->client; //                                       
	...
	if(new_node != NULL)
		...
		case NODE_MMAP:
		{
			...
			tmp->size = record_param->length;
                        ...
                        insert_node_to_list((void **)&buffer->address_using_list,(unsigned int *)new_node); //     
