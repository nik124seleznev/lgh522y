...
#define CCI400_PMU_MAX_HW_EVENTS 5	/*                                          */
...
static int cci_pmu_validate_hw_event(u8 hw_event)
{
	...
	if (ev_source <= CCI400_PMU_EVENT_SOURCE_S4 && ev_code <= CCI400_PMU_EVENT_SLAVE_MAX)
		return hw_event;
	else if (CCI400_PMU_EVENT_SOURCE_M0 <= ev_source &&
		 ev_source <= CCI400_PMU_EVENT_SOURCE_M2 &&
		 CCI400_PMU_EVENT_MASTER_MIN <= ev_code && ev_code <= CCI400_PMU_EVENT_MASTER_MAX)
		return hw_event;
...
{
	return CCI400_PMU_CYCLE_COUNTER_IDX <= idx && idx <= CCI400_PMU_COUNTER_LAST(cci_pmu);
}
...
static int __init cci_probe(void)
{
	...
	for_each_child_of_node(np, cp) {
		...
		if (of_property_read_string(cp, "interface-type", &match_str)) {
