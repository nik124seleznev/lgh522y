		...
		case ALSPS_GET_PS_RAW_DATA_FOR_CALI:
			if(1){
				...
				{	
					...
					else if((pcount1 - pcount2 >= 0x02)||pcount1 > 0x60) //                                                                       
					{
			...
			if(max_ps_value >= 0xF0)
				max_ps_value = 0xF0;
			ps_cali_temp.close = 0xff & max_ps_value;//           
			ps_cali_temp.far_away = 0xff & max_ps_value;//           
...
static int apm_16d_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	atomic_set(&obj->als_suspend, 0);
	atomic_set(&obj->als_cmd_val, 0x0D);//                                    
	...
	{
		atomic_add(0x02, &obj->ps_cmd_val);//                   
		APM_LOG("enable ps interrupt\n");
	}
	if(obj->hw->polling_mode_als == 0)
	{
		atomic_add(0x02, &obj->als_cmd_val);//                    
