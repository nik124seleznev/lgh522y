...
#define INIT_ALSPSMODE_CONTROL    (LEDCURRENT_50MA | ALSGAIN_X1X1)//             
...
/*                                                                           
                                                                           ...
                                                                           */
static struct i2c_client *rpr0521_i2c_client = NULL; /*                                                                   */
static struct rpr0521_priv *g_rpr0521_ptr = NULL; /*                                                                 */
static struct platform_driver rpr0521_alsps_driver;

static int Target_Pdata = 26 /*  */ /*  */ /*  */; /*                                   */
static int NearToFar = 10 /* */ /* */ /*  */; /*                             */
...
static int rpr0521_init_flag = -1;//                   
...
enable_error :
    rpr0521_set_ps_enable(client, old_power_state);//                     
...
enable_error :
    rpr0521_set_als_enable(client, old_power_state);//      
	APS_ERR ( "failed to enable RPR0521\n" );
...
static int rpr0521_do_calibration ( struct i2c_client *client, int *value )
{
    ...
    /*                                                             */
	result = rpr0521_set_control ( client, old_modectl ); /*                           */
    if(result != RPR0521_SUCCESS) {
       cal_running = 0;
       return (result);
    }
...
static unsigned int rpr0521_calc_calibration ( struct i2c_client *client )
{
	...
	result = rpr0521_set_control ( client,INIT_MODE_CONTROL /*                     */); 
    if(result != RPR0521_SUCCESS) {
        return (result);
    }
