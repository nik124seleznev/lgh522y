...
struct apds9930_priv
{
	...
	unsigned int als_status; /*                                                      */
	unsigned int als_th_status; /*                                                                */
...
static int apds9930_clear_interrupt ( struct i2c_client *client, int cmd ) //         
{
	struct apds9930_priv *obj = i2c_get_clientdata ( client );
	int res = 0;
...
static long apds9930_initialize ( struct i2c_client *client  )
{
	...
	/*                                   */
	apds9930_set_atime ( client, 0xED ); /*                                                                           */
	apds9930_set_ptime ( client, 0xFF ); /*                              */
	apds9930_set_wtime ( client, 0xDC ); /*                                         */
	//                                                                                  
	apds9930_set_pers ( client, 0x00|0x00 ); /*                                                                         */
	apds9930_set_config ( client, 0 );

#if defined(TARGET_S6)
    apds9930_set_ppcount ( client, 0x0c ); /*                        */
	apds9930_set_control ( client, 0x24 ); /*                          */
#else
	apds9930_set_ppcount ( client, 0x0a ); /*                        */
	apds9930_set_control ( client, 0x24|0x00 ); /*                           */ /*                      */
#endif
...
static long apds9930_enable ( struct i2c_client *client  )
{
	...
	/*                 */
	res = apds9930_set_enable ( client, 0x2D | 0x12 ); /*             */
	if ( res == APDS9930_SUCCESS )
	{
		/*                           */
...
RE_CALIBRATION:
	...
	/*                              */
	apds9930_set_enable ( client, 0x0D ); /*             */
...
static apds9930_change_ps_threshold(struct i2c_client *client, int int_status)
{
	...
	apds9930_set_pers ( client, 0x20|0x02 ); /*                                     */ /*             */
	/*                                                  */
...
CLEAR_INTERRUPT:	
	...
	/*                    */
	apds9930_set_enable ( client, 0x2D | 0x12 ); /*             */
...
//                                                              
static DRIVER_ATTR ( deviceid, S_IWUSR | S_IRUGO | S_IWGRP | S_IRGRP | S_IROTH, apds9930_show_deviceid, NULL );  //           
//                                                              
