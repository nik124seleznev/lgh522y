...
DEV_INT32 dbg_phy_eyescan(int argc, char** argv){
	if (argc > 10)
	{
		u3phy_ops->eyescan(
			u3phy,
			simple_strtol(argv[1], &argv[1], 10), //    
			simple_strtol(argv[2], &argv[2], 10), //    
			simple_strtol(argv[3], &argv[3], 10), //    
			simple_strtol(argv[4], &argv[4], 10), //    
			simple_strtol(argv[5], &argv[5], 10), //       
			simple_strtol(argv[6], &argv[6], 10), //       
			simple_strtol(argv[7], &argv[7], 10), //       
			simple_strtol(argv[8], &argv[8], 10), //       
			simple_strtol(argv[9], &argv[9], 10), //         
			simple_strtol(argv[10], &argv[10], 10) //              
			);
...
int otg_opt20_b_uut_thread(void *data){
    ...
    */
				...
				if (vbus > 3950 && (os_readl(U3D_SSUSB_OTG_STS) & SSUSB_IDDIG))  //                                         
				{
					//                        
					next_state = OTG_POWERED_STATE;		
					os_printk(K_ERR, "[U3D_T]u3d_get_vbus_mV : raw: %d, change to OTG_POWERED_STATE\n", vbus);
...
//                                                                
					...
					os_writel(U3D_LINK_RESET_INFO, os_readl(U3D_LINK_RESET_INFO) | 0xff00); //                          
					
					os_setmsk(U3D_SSUSB_IP_SPARE0, (0x1<<19)); //                          
