...
#define BOOT_DEBUG_BUF_LEN          (512)                   //          
#define BOOT_TX_MAX_PKT_LEN         (2095- 255)             //                            
...
#define MAGIC_MD_CMD                0x444D434D      //      
#define MAGIC_MD_CMD_ACK            0x4D4B4341      //      

#define SDIOMAIL_BOOTING_REQ        0x53444254      //                                      
#define SDIOMAIL_BOOTING_ACK        0x534254FF      //                      
#define SDIOMAIL_DL_REQ             0x5344444C      //                                    
#define SDIOMAIL_DL_ACK             0x53444CFF      //                      
#define SDIOMAIL_REF                0x52454655      //                         
...
typedef struct EEMCS_MAILBOX_st {
    ...
    wait_queue_head_t h2d_wait_q;               //         
    volatile unsigned int d2h_wakeup_cond;
    volatile unsigned int h2d_wakeup_cond;     //         
} EEMCS_MAILBOX;
/*                                                           */
/*                                                           */
typedef struct {
...
typedef struct{
	KAL_UINT8 check_header[12];	    /*                               */
	KAL_UINT32 header_verno;	        /*                                 */
	KAL_UINT32 product_ver;	        /*                                                     */
	KAL_UINT32 image_type;	            /*                                          */
	KAL_UINT8 platform[16];	        /*                          */
	KAL_UINT8 build_time[64];	        /*                   */
	KAL_UINT8 build_ver[64];	        /*                                   */
	KAL_UINT8 bind_sys_id;	            /*                                           */
	KAL_UINT8 reserved[3];             /*              */
	KAL_UINT32 reserved_info[3];       /*              */
	KAL_UINT32 size;	                /*                            */
}MD_CHECK_HEADER;
...
typedef struct{
    ...
    KAL_UINT8  mode[36];	           /*                                   */
    KAL_UINT8  moly_week[16];	       /*                                  */
    KAL_UINT32 reserved_2[12];       /*              */
...
typedef struct EEMCS_BOOT_SET_st {
    ...
    KAL_UINT8 *cmd_buff;                /*                                     */
    KAL_UINT8 *debug_buff;              /*                               */
    KAL_UINT32 debug_offset;            /*                         */
    KAL_UINT32 cb_id;
    KAL_UINT32 ccci_hs_bypass;          /*                            */

    struct class *dev_class;            /*                                                         */
    struct cdev *eemcs_boot_chrdev;     /*                                        */
    eemcs_cdev_node_t boot_node;
    KAL_INT32 expt_cb_id;               /*                                */
    wait_queue_head_t  rst_waitq;       /*                                                      */
    MD_INFO  md_info;
    KAL_UINT32 md_need_reload;          /*                                    */
    char ext_modem_post_fix[EXT_MD_POST_FIX_LEN]; /*                                     */
    struct wake_lock	eemcs_boot_wake_lock;
    KAL_UINT8  eemcs_boot_wakelock_name[32];
    KAL_UINT32 md_id;                               /*                                */
    KAL_UINT32 md_img_exist_list[MD_IMG_MAX_CNT+1]; /*                                   */
