...
#define EEMCS_BOOT_UT_BL_CNT (1)            //                                                  
#if (EEMCS_BOOT_UT_BL_CNT >= 2)
#define EEMCS_BOOT_UT_BL_LOOP_START (12)    //                                                       
#define EEMCS_BOOT_UT_BL_LOOP_END   (22)    //                                                     
#endif
...
KAL_INT32 eemcs_boot_rx_callback(struct sk_buff *skb, KAL_UINT32 private_data)
{
    ...
    if (CDEV_OPEN == atomic_read(&boot_node->cdev_state)) {
        skb_queue_tail(&boot_node->rx_skb_list, skb);    //                                             
        atomic_inc(&boot_node->rx_pkt_cnt);              //                    
...
//                                                       
//                                                                 
            DBGLOG(BOOT, INF, "MD Image Loading..");  //                   
        }
//                    
    };
#ifdef _EEMCS_BOOT_DEBUG_IMG
...
KAL_INT32 eemcs_md_reset(void)
{
    ...
    wake_lock_timeout(&eemcs_boot_inst.eemcs_boot_wake_lock, 10*HZ); //             
