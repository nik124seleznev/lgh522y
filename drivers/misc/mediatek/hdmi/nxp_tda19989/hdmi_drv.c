...
void _tda19989_hdcp_on(tda_instance *this) {
	...
	if (this->tda.hdcp_status != HDCP_IS_NOT_INSTALLED) {	/*                             */
		if (this->tda.hdcp_enable) {	/*                     */
			TDA_TRY(tmdlHdmiTxSetHdcp(this->tda.instance, True));	/*              */
#if defined(TMFL_TDA19989) || defined(TMFL_TDA9984)
			/*                                                            */
...
void _tda19989_hdcp_off(tda_instance *this) {
	...
	if (this->tda.hdcp_status != HDCP_IS_NOT_INSTALLED) {	/*                             */
		...
		if (this->tda.hdcp_enable) {	/*                       */
			TDA_TRY(tmdlHdmiTxSetHdcp(this->tda.instance, False));	/*               */
		}
	}
 TRY_DONE:
	(void)0;
...
tmdlHdmiTxVinMode_t tda19989_vin_format_convert(HDMI_VIDEO_INPUT_FORMAT vin) {
	...
	return TMDL_HDMITX_VINMODE_INVALID;	/*               */
}
...
tmdlHdmiTxVoutMode_t tda19989_vout_format_convert(HDMI_VIDEO_OUTPUT_FORMAT vout) {
	...
	return TMDL_HDMITX_VOUTMODE_INVALID;	/*               */
}
...
static int hdmi_drv_video_config(HDMI_VIDEO_RESOLUTION vformat, HDMI_VIDEO_INPUT_FORMAT vin,
...
#ifdef TMFL_TDA19989
	this->tda.setio.video_out.dviVqr = TMDL_HDMITX_VQR_DEFAULT;	/*                               */
#endif
	/*                                                                            */
	/*                                                                          */
	/*                                                                          */
	...
	this->tda.setio.sink = TMDL_HDMITX_SINK_HDMI;	/*                   */
...
static int hdmi_drv_audio_config(HDMI_AUDIO_FORMAT aformat) {
	...
	this->tda.setio.audio_in.dstRate = TMDL_HDMITX_DSTRATE_SINGLE;	/*                   */
	this->tda.setio.audio_in.channelAllocation = 0;	/*                                                */
	/*                                                */
