...
//           
static uint8_t hidbi = 0;                    //                 
static uint8_t hidbt = 0;                    //                
static uint8_t hidbuf[HID_BUF * HID_SIZE];    //                     

//      
static __u32 mouse_btns[] = { BTN_LEFT, BTN_MIDDLE, BTN_RIGHT }; //          
int mouse_btn = 0;
...
const char print_kb_map[KB_LEN][32] = {  //                                                                 
...
const unsigned int kb_map[KB_LEN] = {  //                                                                 
  0,0,0,0, KEY_A, KEY_B, KEY_C, KEY_D, KEY_E, KEY_F, 
  KEY_G, KEY_H, KEY_I, KEY_J, KEY_K, KEY_L, KEY_M, KEY_N, KEY_O, KEY_P,
  KEY_Q, KEY_R, KEY_S, KEY_T, KEY_U, KEY_V, KEY_W, KEY_X, KEY_Y, KEY_Z, 
  KEY_1, KEY_2, KEY_3, KEY_4, KEY_5, KEY_6, KEY_7, KEY_8, KEY_9, KEY_0,
...
int64_t GetTimeStamp(void){
    struct timeval tv;
    do_gettimeofday(&tv);
    return (tv.tv_sec & 0xff) * 1000000 + tv.tv_usec; /*                      */
}
...
static int smartbook_init(int flag) {
    ...
    if(smartbook_dev==NULL) {
        ...
        set_bit(EV_KEY, smartbook_dev->evbit); //                  
        //                                       
        set_bit(EV_REL, smartbook_dev->evbit); //          
...
int smartbook_kthread(void *data) {
    ...
    while(1) {
        ...
        mutex_lock(&smb_mutex);
        if(smartbook_dev && retval!=0 && hidbt!=hidbi) SiiHidRead(); //            
        mutex_unlock(&smb_mutex);
