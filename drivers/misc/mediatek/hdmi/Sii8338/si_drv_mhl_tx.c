...
static void WriteInitialRegisterValues(void)
{
	...
	SiiRegWrite(TX_PAGE_3 | 0x0018, 0x04);	/*                                                   */
...
#ifdef MDT_SUPPORT		/*                     */
	SiiRegWrite(TX_PAGE_CBUS | 0x00F0, MHL_INT_REQ_WRT | MHL_INT_DCAP_CHG);	/*                                    */
...
uint8_t siMhlTx_VideoAudioSet(void)
{
	...
	HalTimerWait(T_RES_CHANGE_DELAY);	/*                                                              */
	/*                     */
	AudioVideoIsr(true);
	/*                 */
