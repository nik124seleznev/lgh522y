...
struct input_dev *get_native_touchscreen_dev(void);	/*                                                        */
void release_native_touchscreen_dev(void);	/*                                    */
#endif				/*                                                            */
...
#else
code const uint8_t g_dpad_keys[4] = {	/*                                 */
#endif
...
#else
code const uint8_t usb_kbd_keycode[256] = {	/*                                 */
#endif				/*                                                            */
	/*                             */
...
*/
	if (buttons != mdt.prior_mouse_buttons) {
		si_input_report_key(dev_mouse, BTN_LEFT, buttons & MDT_BUTTON_LEFT);	/*                                  */
		si_input_report_key(dev_mouse, BTN_RIGHT, buttons & MDT_BUTTON_RIGHT);	/*                              */
		si_input_report_key(dev_mouse, BTN_MIDDLE, buttons & MDT_BUTTON_MIDDLE);
		mdt.prior_mouse_buttons = buttons;
	}

	if (x)
		si_input_report_rel(dev_mouse, REL_X, x);	/*                                         */
	if (y)
...
void mdt_generate_event_gamepad(struct mdt_cursor_other_t *gamePacket)
{
	...
	uint8_t X = gamePayload->xyzRz_byteLen[MDT_GAMEXYRZ_X];	/*                     */
	uint8_t Y = gamePayload->xyzRz_byteLen[MDT_GAMEXYRZ_Y];	/*                     */
	...
	if ((mdt.prior_game_event.dpad_event == MDT_HID_DPAD_IDLE)
		...
		switch (dpad_event) {
		case MDT_HID_DPAD_000_DEGREES:	/*           */
			dpad_key_bits = MDT_DPAD_UP;
			break;
		case MDT_HID_DPAD_045_DEGREES:	/*            */
			dpad_key_bits = (MDT_DPAD_UP | MDT_DPAD_RIGHT);
			break;
		case MDT_HID_DPAD_090_DEGREES:	/*            */
			dpad_key_bits = MDT_DPAD_RIGHT;
			break;
		case MDT_HID_DPAD_135_DEGREES:	/*             */
			dpad_key_bits = (MDT_DPAD_RIGHT | MDT_DPAD_DOWN);
			break;
		case MDT_HID_DPAD_180_DEGREES:	/*             */
			dpad_key_bits = MDT_DPAD_DOWN;
			break;
		case MDT_HID_DPAD_225_DEGREES:	/*             */
			dpad_key_bits = MDT_DPAD_DOWN | MDT_DPAD_LEFT;
			break;
		case MDT_HID_DPAD_270_DEGREES:	/*             */
			dpad_key_bits = MDT_DPAD_LEFT;
			break;
		case MDT_HID_DPAD_315_DEGREES:	/*             */
		...
		for (i = 0; i < 4; i++)	/*                                                 */
			input_report_key(mdt.dev_keyboard, g_dpad_keys[i], (dpad_key_bits & (1 << i)));	/*          */
...
static unsigned char doubletouch_triggered_mouse_button_simulation(unsigned char isTouched)
{
	...
	case MDT_PB_WAIT_FOR_1ST_TOUCH_RELEASE:
		...
		mdt.double_touch.last_release = jiffies_to_msecs(jiffies);
		if (mdt.double_touch.duration_touch < ONE_SECOND)	/*                               */
	...
	case MDT_PB_WAIT_FOR_2ND_TOUCH_RELEASE:	/*                                          */
		...
		mdt.double_touch.last_release = jiffies_to_msecs(jiffies);
		if (mdt.double_touch.duration_touch < ONE_SECOND) {	/*                               */
			si_input_report_key(dev_mouse, BTN_LEFT, 1);	/*                  */
...
unsigned char mdt_generate_event_mouse_from_nativetouch(struct mdt_touch_history_t *touch_event)
{
	...
	if (touch_event->isTouched == 0)
		...
		if (mdt.touch_debounce_counter < 5) {	/*                                        */
			...
			case MDT_PB_SIMULATED_MOUSE_270:
				...
				input_event_mouse(mouse_buttons, deltaX, deltaY, 0);
				break;
			case MDT_PB_NATIVE_TOUCH_SCREEN:	/*                                         */
				break;
			}
#endif
		}
...
void mdt_generate_event_keyboard(struct mdt_keyboard_event_t *keyboardPacket)
{
	...
	/*                                              */
	for (i = 0; i < 3; i++)	/*                                              */
		si_input_report_key(dev_keyboard, usb_kbd_keycode[i + 224], (keycodes_new[0] >> i) & 1);	/*          */
	...
	memcpy(keycodes_old, keycodes_new, 7);	/*                            */
}
