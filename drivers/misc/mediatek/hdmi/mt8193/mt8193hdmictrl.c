...
void vHalHDMI_NCTS(u8 bAudioFreq, u8 bPix, u8 bDeepMode)
{
	...
	if (bDeepMode == HDMI_NO_DEEP_COLOR) {
		...
		u4NTemp = (bData1[4] << 16) | (bData1[5] << 8) | (bData1[6]);	/*   */
...
void vSendAVIInfoFrame(u8 ui1resindex, u8 ui1colorspace)
{
	...
	_bAviInfoFm[2] = 0;	/*        */
	_bAviInfoFm[2] |= 0x04;	/*             */
	_bAviInfoFm[3] = HDMI_VIDEO_ID_CODE[ui1resindex];	/*        */
...
void vSend_AVUNMUTE(void)
{
	...
	bData |= CFG4_AV_UNMUTE_EN;	/*                       */
	bData &= ~CFG4_AV_UNMUTE_SET;	/*         */
