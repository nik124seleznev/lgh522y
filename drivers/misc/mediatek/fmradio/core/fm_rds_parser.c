...
static fm_s32 rds_g0_ps_cmp(fm_u8 addr, fm_u16 cbc, fm_u8 *fresh,
			    fm_u8 *once, fm_u8 *twice, /*                */ fm_u8 *bm)
{
	fm_s32 ret = 0, indx;
	/*               */
	/*               */
	...
	if (!SubType) {
		...
		if (valid == fm_false) {
			...
			if ((AF_H > 224) && (AF_H < 250)) {
				...
				if ((AF_L < 205) && (AF_L > 0)) {
					/*                                  */
					pstRDSData->AF_Data.AF[0][0] = AF_L + 875;	/*                   */
	...
	if (!SubType) {
		...
		if ((block_data[4] & FM_RDS_GDBK_IND_C) && (block_data[4] & FM_RDS_GDBK_IND_D)) {
			...
			if (block_data[3] & 0x0020) {
				pstRDSData->CT.Local_Time_offset_signbit = 1;	/*          */
			}
		  ...
		  fm_u16(*getfreq) (void))
{
	...
	while (rds_cnt > 0) {
		...
		if (ret) {
			...
			ret = rds_event_set(event, RDS_EVENT_PI_CODE);	/*                         */
		}
		...
		if (ret) {
			...
			ret = rds_event_set(event, RDS_EVENT_PTY_CODE);	/*                          */
		}
