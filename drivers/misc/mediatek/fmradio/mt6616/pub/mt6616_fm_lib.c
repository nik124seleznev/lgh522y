...
static fm_s32 mt6626_ClearSTCDone(void)
{
	...
	mt6626_read(FM_MAIN_INTR, &dataRead);
	mt6626_write(FM_MAIN_INTR, dataRead | FM_INTR_STC_DONE);	/*                   */
	return 0;
}
...
static fm_s32 mt6626_RampDown(void)
{
	...
	/*                 */
	mt6626_read(FM_MAIN_CTRL, &dataRead);
	mt6626_write(FM_MAIN_CTRL, (dataRead & 0xFFF0));	/*                                   */
...
static fm_s32 mt6626_DspPatch(const fm_u16 *img, enum IMG_TYPE type)
{
	...
	mt6626_write(FM_DSP_PATCH_OFFSET, img[0]);	/*               */
	mt6626_write(FM_DSP_PATCH_CTRL, 0x40);	/*                        */
	mt6626_write(FM_DSP_PATCH_CTRL, ctrl_code);	/*                      */
...
static fm_s32 mt6626_PowerDown(void)
{
  ...
  */
	...
	if (dataRead & 0x1) {
		mt6626_write(FM_MAIN_INTR, dataRead);	/*                   */
	}
			   ...
			   fm_u16 space)
{
	...
	mt6626_write(FM_MAIN_CFG2, (dataRead & 0xFC00) | ((min_freq - 640) << 1));	/*                                                   */
	mt6626_read(FM_MAIN_CFG1, &dataRead);
	mt6626_write(FM_MAIN_CFG1, (dataRead & 0x8800) | (seekdir << 10) | (1 << (12 + space)) | ((max_freq - 640) << 1));	/*                                                   */
	mt6626_read(FM_MAIN_CFG1, &dataRead);
	mt6626_write(FM_MAIN_CFG1, (dataRead & 0xF7FF) | 0x0800);	/*                                               */
			   ...
			   fm_u16 *pScanTBL, fm_u16 *ScanTBLsize, fm_u16 scandir, fm_u16 space)
{
	...
	mt6626_write(FM_MAIN_CFG2, (dataRead & 0xFC00) | ((min_freq - 640) << 1));	/*                                                   */
	mt6626_read(FM_MAIN_CFG1, &dataRead);
	mt6626_write(FM_MAIN_CFG1, (dataRead & 0x8800) | (scandir << 10) | (1 << (12 + space)) | ((max_freq - 640) << 1));	/*                                                   */
	mt6626_read(FM_MAIN_CFG1, &dataRead);
	mt6626_write(FM_MAIN_CFG1, dataRead & 0xF7FF);	/*                                            */
