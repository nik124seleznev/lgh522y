...
fm_s32 mt6628_pwrup_fpga_on(fm_u8 *buf, fm_s32 buf_size)
{
	...
	pkt_size += fm_bop_udelay(30000, &buf[pkt_size], buf_size - pkt_size);	/*            */
	pkt_size += fm_bop_write(0x01, 0x6A00, &buf[pkt_size], buf_size - pkt_size);
	pkt_size += fm_bop_udelay(50000, &buf[pkt_size], buf_size - pkt_size);	/*            */
	...
	pkt_size += fm_bop_udelay(200000, &buf[pkt_size], buf_size - pkt_size);	/*             */
	pkt_size += fm_bop_write(0x1B, 0x0094, &buf[pkt_size], buf_size - pkt_size);
	pkt_size += fm_bop_write(0x00, 0x0167, &buf[pkt_size], buf_size - pkt_size);
...
fm_s32 mt6628_pwrup_clock_on(fm_u8 *buf, fm_s32 buf_size)
{
	...
	de_emphasis = mt6628_fm_config.rx_cfg.deemphasis;	/*                                                   */
	de_emphasis &= 0x0001;	/*          */
	osc_freq = mt6628_fm_config.rx_cfg.osc_freq;	/*                                                 */
	osc_freq &= 0x0007;	/*          */
	...
	pkt_size += fm_bop_write(0x60, 0x0001, &buf[pkt_size], buf_size - pkt_size);	/*            */
	...
	pkt_size += fm_bop_modify(0x70, 0xFFBF, 0x0040, &buf[pkt_size], buf_size - pkt_size);	/*              */
	/*                                                 */
...
fm_s32 mt6628_pwrup_digital_init(fm_u8 *buf, fm_s32 buf_size)
{
	...
	/*                                                        */
	pkt_size += fm_bop_write(0x6A, 0x2100, &buf[pkt_size], buf_size - pkt_size);	/*            */
	pkt_size += fm_bop_write(0x6B, 0x2100, &buf[pkt_size], buf_size - pkt_size);	/*            */
	pkt_size += fm_bop_modify(0x60, 0xFFF7, 0x0008, &buf[pkt_size], buf_size - pkt_size);	/*            */
