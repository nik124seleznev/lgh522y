...
struct fm {
	...
	fm_s32 ref;		/*                               */
	fm_bool chipon;		/*                  */
	enum fm_pwr_state pwr_sta;	/*                       */
	enum fm_op_state op_sta;	/*                                               */
	/*                                                 */
	fm_s32 vol;		/*                                     */
	fm_bool mute;		/*                            */
	fm_bool rds_on;		/*                      */
	enum fm_antenna_type ana_type;	/*                    */
	fm_bool via_bt;		/*                                                  */
	fm_u16 min_freq;	/*                */
	fm_u16 max_freq;	/*                 */
	fm_u16 cur_freq;	/*                   */
...
struct fm_basic_interface {
	...
	fm_s32(*dumpreg) (void);
	fm_bool(*msget) (fm_u16 *ms);	/*                           */
	fm_s32(*msset) (fm_s32 ms);	/*                       */
	...
	fm_s32(*is_dese_chan) (fm_u16 freq);	/*                                     */
	fm_s32(*softmute_tune) (fm_u16 freq, fm_s32 *rssi, fm_bool *valid);
	fm_s32(*pre_search) (void);
	fm_s32(*restore_search) (void);
	fm_s32(*desense_check) (fm_u16 freq, fm_s32 rssi);	/*                                  */
	fm_s32(*get_freq_cqi) (fm_u16 freq, fm_s32 *cqi);
	fm_s32(*cqi_log) (fm_s32 min_freq, fm_s32 max_freq, fm_s32 space, fm_s32 cnt);	/*              */
	fm_s32(*fm_via_bt) (fm_bool flag);	/*                               */
...
struct fm_rds_interface {
	...
	fm_s32(*rds_parser) (rds_t *rds_dst, struct rds_rx_t *rds_raw, fm_s32 rds_size,
			     fm_u16(*getfreq) (void));
	fm_u16(*rds_gbc_get) (void);	/*                    */
	fm_u16(*rds_bbc_get) (void);	/*                   */
	fm_u8(*rds_bbr_get) (void);	/*                 */
	fm_s32(*rds_bc_reset) (void);	/*                     */
	fm_u32(*rds_bci_get) (void);	/*                     */
