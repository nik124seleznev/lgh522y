...
static int led_breath_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting* led)
{
	...
	switch(pmic_type){
		...
		case MT65XX_LED_PMIC_NLED_ISINK1://               
			upmu_set_isinks_ch1_mode(PMIC_PWM_1);//           
			upmu_set_isinks_ch1_step(0x3);//    
			upmu_set_isinks_breath0_trf_sel(0x04);
			upmu_set_isinks_breath0_ton_sel(0x02);
			upmu_set_isinks_breath0_toff_sel(0x03);
			upmu_set_isink_dim1_duty(15);
...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting* led) {
	...
	switch(pmic_type){
		case MT65XX_LED_PMIC_NLED_ISINK0:
			...
			upmu_set_isink_ch0_step(0x1);//   
			#else
			upmu_set_isink_ch0_step(0x3);//    
			#endif
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	if (pmic_type == MT65XX_LED_PMIC_LCD_ISINK)
	{
		if(backlight_init_flag == false)
		{
            ...
            upmu_set_isink_ch0_step(0x5); //     
            upmu_set_isink_sfstr0_en(0x0); //                   
			upmu_set_rg_isink0_double_en(0x1); //                      
			upmu_set_isink_phase_dly_tc(0x0); //           
			upmu_set_isink_phase0_dly_en(0x1); //                   
            upmu_set_isink_chop0_en(0x1); //                
            //       
            upmu_set_rg_isink1_ck_pdn(0x0); //                      
            upmu_set_rg_isink1_ck_sel(0x1); //                          
			upmu_set_isink_ch1_mode(ISINK_PWM_MODE);
            upmu_set_isink_ch1_step(0x0); //    
            upmu_set_isink_sfstr1_en(0x0); //                   
			upmu_set_rg_isink1_double_en(0x1); //                      
			upmu_set_isink_phase1_dly_en(0x1); //                   
            upmu_set_isink_chop1_en(0x1); //                         
            //       
            upmu_set_rg_isink2_ck_pdn(0x0); //                      
            upmu_set_rg_isink2_ck_sel(0x1); //                          
			upmu_set_isink_ch2_mode(ISINK_PWM_MODE);
            upmu_set_isink_ch2_step(0x5); //     
            upmu_set_isink_sfstr2_en(0x0); //                   
			upmu_set_rg_isink2_double_en(0x1); //                      
			upmu_set_isink_phase2_dly_en(0x1); //                   
            upmu_set_isink_chop2_en(0x1); //                   
            //       
            upmu_set_rg_isink3_ck_pdn(0x0); //                      
            upmu_set_rg_isink3_ck_sel(0x1); //                          
			upmu_set_isink_ch3_mode(ISINK_PWM_MODE);
            upmu_set_isink_ch3_step(0x5); //     
            upmu_set_isink_sfstr3_en(0x0); //                   
			upmu_set_rg_isink3_double_en(0x1); //                      
			upmu_set_isink_phase3_dly_en(0x1); //                   
            upmu_set_isink_chop3_en(0x1); //                                
		...
		if (level) 
		{
            ...
            upmu_set_isink_dim0_fsel(0x2); //      
            upmu_set_isink_dim1_fsel(999); //      
            upmu_set_isink_dim2_fsel(0x2); //      
            upmu_set_isink_dim3_fsel(0x2); //                  
		...
		else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK3)
		{
			if((button_flag_isink3==0) && (first_time == true)) {//                                                             
				...
				upmu_set_rg_isink3_ck_sel(0);
				upmu_set_isink_ch3_mode(PMIC_PWM_0);
				upmu_set_isink_ch3_step(0x0);//   
				//                                
				upmu_set_isink_dim3_duty(31);
				upmu_set_isink_dim3_fsel(999);//         
...
int mt_mt65xx_breath_mode(unsigned int patternid)
{
	...
	upmu_set_isink_ch1_step(led_scene_setting[patternid].isink1_step);//   
	...
	upmu_set_isink_ch0_step(led_scene_setting[patternid].isink1_step);//   
