...
#define MVA_BLOCK_SIZE_ORDER     18     //    
...
#define MAX_BUF_SIZE_TO_GET_USER_PAGE (200*1024*1024)  //                                   
...
static unsigned int pt_pa_nonsec;    //                                      
...
typedef enum
{
	M4U_TEST_LEVEL_USER = 0,  //                                     
	M4U_TEST_LEVEL_ENG = 1,   //                                 
...
static int MTK_M4U_release(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    if(pNode->isM4uDrvConstruct==0 || pNode->isM4uDrvDeconstruct==0 || !list_empty(&pNode->mvaList))
    {
        ...
        M4UMSG("open->tgid=%d, cur->tgid=%d, cur->mm=0x%x, inode=%p, file=%p.\n",
...
//                                                                      
    ...
    for(i=m4u_index_offset;i<m4u_index_offset+SEQ_RANGE_NUM;i++)
    {
        if(1==pRangeDes[i].Enabled)
        {
            if(MVAEnd<pRangeDes[i].MVAStart || MVAStart>pRangeDes[i].MVAEnd) //          
            {
            	  continue;
            }
            else
...
err:
    for(i--; i>=0; i--)
    {
        pa = round_down(*(pPhys+i) , PAGE_SIZE);
    ...
    unsigned int BufSize, unsigned int MVA, struct sg_table* sg_table)
{
    ...
    if(BufAddr<PAGE_OFFSET && sg_table==NULL)  //                
    {	
...
//                                                                   
    ...
    if(size < BUFFER_SIZE_FOR_FLUSH_ALL)
    {
        ...
        if((unsigned int)start<PAGE_OFFSET)  //                
        {
        ...
        else if((unsigned int)start>=VMALLOC_START && (unsigned int)start<=VMALLOC_END) //        
        {
...
int m4u_do_dma_cache_maint(M4U_MODULE_ID_ENUM eModuleID, const void *va, size_t size, int direction)
{
    ...
    if((unsigned int)va<PAGE_OFFSET)  //                
    {
