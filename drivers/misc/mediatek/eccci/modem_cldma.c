...
static void cldma_rx_done(struct work_struct *work)
{
    ...
    cldma_write32(md_ctrl->cldma_ap_base, CLDMA_AP_SO_RESUME_CMD, CLDMA_BM_ALL_QUEUE&(1<<queue->index));
        cldma_read32(md_ctrl->cldma_ap_base, CLDMA_AP_SO_RESUME_CMD); //           
    //                         
...
static inline void cldma_stop(struct ccci_modem *md)
{
    ...
    do {
        ...
        cldma_read32(md_ctrl->cldma_ap_base, CLDMA_AP_UL_STOP_CMD); //           
    ...
    do {
        ...
        cldma_read32(md_ctrl->cldma_ap_base, CLDMA_AP_SO_STOP_CMD); //           
...
static inline void cldma_start(struct ccci_modem *md)
{
    ...
    cldma_read32(md_ctrl->cldma_ap_base, CLDMA_AP_UL_START_CMD); //           
    md_ctrl->txq_active |= CLDMA_BM_ALL_QUEUE;
    cldma_write32(md_ctrl->cldma_ap_base, CLDMA_AP_SO_START_CMD, CLDMA_BM_ALL_QUEUE);
    cldma_read32(md_ctrl->cldma_ap_base, CLDMA_AP_SO_START_CMD); //           
    md_ctrl->rxq_active |= CLDMA_BM_ALL_QUEUE;
    //                                    
...
static void md_cd_wdt_work(struct work_struct *work)
{
    ...
    if(*((int *)(md->mem_layout.smem_region_vir+CCCI_SMEM_OFFSET_EPON)) == 0xBAEBAE10) { //        
        ...
        CCCI_INF_MSG(md->index, TAG, "mdlogger check: WDT actvie[2]= %d \n", md->critical_user_active[2]);
...
retry:
    ...
    if(queue->free_slot > 0) {
        ...
        if(md_ctrl->txq_active & (1<<qno)) {
            ...
            cldma_read32(md_ctrl->cldma_ap_base, CLDMA_AP_UL_RESUME_CMD); //                                          
        }
...
static int md_cd_send_runtime_data(struct ccci_modem *md, unsigned int sbp_code)
{
	...
	else
	{
		...
		memcpy((char*)&(runtime->feature_4_val[3]), temp_buf_svn, 4); /*     */
	...
	else
	{
		...
		memcpy((char*)&(runtime->feature_4_val[0]), mcc, 4); /*     */
		memcpy((char*)&(runtime->feature_4_val[1]), mnc, 4); /*     */
		memcpy((char*)&(runtime->feature_4_val[2]), subset, 4); /*        */
