...
static void hw_bc11_dump_register(void)
{
 ...
 static kal_uint32 charging_hw_init(void *data)
 {
	...
	upmu_set_rg_csdac_mode(1);      //          
	upmu_set_rg_vbat_ov_en(1);      //          
#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
	upmu_set_rg_vbat_ov_vth(0x2);   //                  
#else
	upmu_set_rg_vbat_ov_vth(0x1);   //                  
#endif
	...
	upmu_set_rg_ulc_det_en(1);      //                
	upmu_set_rg_low_ich_db(1);      //                       


#if defined(CONFIG_MTK_PUMP_EXPRESS_SUPPORT)
	upmu_set_rg_csdac_dly(0x0); 			//          
	upmu_set_rg_csdac_stp(0x1); 			//          
	upmu_set_rg_csdac_stp_inc(0x1); 		//              
	upmu_set_rg_csdac_stp_dec(0x7); 		//              
	upmu_set_rg_cs_en(1);					//       
	
	upmu_set_rg_hwcv_en(1); 		
	upmu_set_rg_vbat_cv_en(1);				//      
	upmu_set_rg_csdac_en(1);				//         
	upmu_set_rg_chr_en(1);					//       
#endif
 ...
 static kal_uint32 charging_enable(void *data)
 {
	...
	if(KAL_TRUE == enable)
	{
	    upmu_set_rg_csdac_dly(0x4);             //          
	    upmu_set_rg_csdac_stp(0x1);             //          
	    upmu_set_rg_csdac_stp_inc(0x1);         //              
	    upmu_set_rg_csdac_stp_dec(0x2);         //              
	    upmu_set_rg_cs_en(1);                   //                
	...
	else
	{
	    ...
	    upmu_set_rg_chrwdt_flag_wr(0);   //            
	    
	    upmu_set_rg_csdac_en(0);         //         
