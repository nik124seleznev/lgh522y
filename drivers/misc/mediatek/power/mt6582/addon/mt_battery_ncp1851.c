...
void select_charging_curret_bcct(void)
{
    if (BMT_status.charger_type == STANDARD_HOST)
    {
        ...
        ncp1851_set_iinlim(0x1); //                             
        ncp1851_set_iinset_pin_en(0x0); //                                           
        ncp1851_set_iinlim_en(0x1); //                          
        ncp1851_set_aicl_en(0x0); //                       
             ...
             (BMT_status.charger_type == NONSTANDARD_CHARGER))
    {
        ...
        else if(g_bcct_value < 700)        
        {
            ...
            ncp1851_set_iinlim(0x2); //                             
            ncp1851_set_iinset_pin_en(0x0); //                                           
            ncp1851_set_iinlim_en(0x1); //                          
            ncp1851_set_aicl_en(0x1); //            
            ncp1851_set_ichg(g_temp_CC_value);       	
        }    
...
void ncp1851_set_low_chg_current(void)
{
    ...
    ncp1851_set_iinlim(0x0); //                          
    ncp1851_set_iinset_pin_en(0x0); //                                           
    ncp1851_set_iinlim_en(0x1); //                              
    ncp1851_set_aicl_en(0x0); //            
    ncp1851_set_ichg(0x0); //                               
}
...
//                                                    
    ...
    if(g_temp_status == TEMP_NEG_10_TO_POS_0)
    {    
        ncp1851_set_ctrl_vbat(0x1C); //           
    }
    else
    {
        if(g_temp_status == TEMP_POS_10_TO_POS_45)
        {
            if((ncp1851_status == 0x8) || (ncp1851_status == 0x9) || (ncp1851_status == 0xA)) //                                 
                ncp1851_set_ctrl_vbat(0x1C); //           
            else if(ncp1851_status == 0x4)
                ncp1851_set_ctrl_vbat(0x28); //                                   
            else
                ncp1851_set_ctrl_vbat(0x24); //           
        }
        else
            ncp1851_set_ctrl_vbat(0x20); //           
    }     
#else
    if((ncp1851_status == 0x8) || (ncp1851_status == 0x9) || (ncp1851_status == 0xA)) //                                 
        ncp1851_set_ctrl_vbat(0x1C); //           
    ...
    ncp1851_set_ieoc(0x4); //                                                          

    if((BMT_status.charger_type != STANDARD_HOST) && (BMT_status.charger_type != CHARGER_UNKNOWN))
        ncp1851_set_iweak(0x3); //                           
    else
        ncp1851_set_iweak(0x1); //                           

    ncp1851_set_ctrl_vfet(0x3); //            
    ncp1851_set_vchred(0x2); //                    
    ncp1851_set_ichred(0x0); //                    
...
int BAT_CheckBatteryStatus_ncp1851(void)
{
    ...
    /*                 */
    BMT_status.ADC_I_SENSE = get_i_sense_volt(10); //               
    BMT_status.bat_vol = BMT_status.ADC_I_SENSE;

    /*               */
    BMT_status.ADC_BAT_SENSE = get_bat_sense_volt(10); //              
...
PMU_STATUS BAT_BatteryFullAction(void)
{
    ...
    {
#if defined(MTK_JEITA_STANDARD_SUPPORT)
        if(BMT_status.bat_vol > g_jeita_recharging_voltage + 70 )  // 
#else    
...
void pmic_init_for_ncp1851(void)
{
    ...
    upmu_set_rg_csdac_mode(0);      //          

    upmu_set_rg_vcdt_mode(0);       //         
    upmu_set_rg_vcdt_hv_en(1);      //          
    ...
    upmu_set_rg_vbat_ov_vth(0x0);   //                     
//                                              

    //             
//                                                  
    upmu_set_rg_baton_ht_en(0);     //           
    
    upmu_set_rg_ulc_det_en(0);      //                
    upmu_set_rg_low_ich_db(1);      //                       

    upmu_set_rg_chr_en(0);           //       
    upmu_set_rg_hwcv_en(0);          //           
...
void BAT_thread_ncp1851(void)
{
    ...
    else
    {
        ...
        /*                    */
        if((BMT_status.bat_vol > RECHARGING_VOLTAGE) && ((ncp1851_status == 0x6) || (ncp1851_status == 0x7))) //                                                       
        {
        ...
        else if((ncp1851_status == 0x6) || (ncp1851_status == 0x7)) //                         
        {
            pchr_turn_off_charging_ncp1851();
            msleep(100);
            if (Enable_BATDRV_LOG == 1) {
...
void PrechargeCheckStatus(void)
{
    ...
    while(1)
    {
        if((g_bat_init_flag == 1) && (g_pmic_init_for_ncp1851 == 1)) //                                                
            break;
        else
            msleep(1000);
    }
