...
static int amdtxpwr_backoff(int level)
{
    ...
    else if (level >= 1 && level <= COOLER_STEPS-1)
    {
        //                            
        //                                                             
        ret = eemcs_notify_md_by_sys_msg(MD_SYS5, EXT_MD_TX_PWR_REDU_REQ, (23- (level)*2)); //                         
...
int amdtxctrl_param_read( char *buf, char **start, off_t offset, int count, int *eof, void *data )
{
	...
	sprintf(tmp, "[up]\t%3d(sec)\t%2d\n[low]\t%3d(sec)\t%2d\nrst=%2d\ninterval=%d\nmax_step=%d\n", up_duration, up_step, \
								low_duration, low_step, low_rst_max, polling_interval, amdtxctrl.max_level);
	ret = strlen(tmp);

	memcpy(buf, tmp, ret*sizeof(char));

	mtk_cooler_amdtxctrl_dprintk_always("[%s] [up]%d %d, [low]%d %d, rst=%d, interval=%d, max_step=%d\n", __func__, up_duration, \
		up_step, low_duration, low_step, low_rst_max, polling_interval, amdtxctrl.max_level);
...
ssize_t amdtxctrl_param_write( struct file *filp, const char __user *buf, unsigned long len, void *data )
{
	...
	if (sscanf(desc, "%d %d %d %d %d %d %d", &tmp_up_dur, &tmp_up_step, &tmp_low_dur, \
		...
		if (tmp_deepest_step > 0 && tmp_deepest_step < COOLER_STEPS)
		    amdtxctrl.max_level = tmp_deepest_step;

		over_up_time = 0;
		below_low_time = 0;
		low_rst_time = 0;

		mtk_cooler_amdtxctrl_dprintk_always("[%s] %s [up]%d %d, [low]%d %d, rst=%d, interval=%d, max_step=%d\n", __func__, desc, up_duration, \
			up_step, low_duration, low_step, low_rst_max, polling_interval, amdtxctrl.max_level);
