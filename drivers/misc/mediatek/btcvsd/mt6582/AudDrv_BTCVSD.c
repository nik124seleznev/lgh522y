...
#define INFRA_MISC (INFRA_BASE + 0x00001F00)    //                                                                                         
...
static int AudDrv_btcvsd_probe(struct platform_device *dev)
{
    ...
    ret = request_irq(MT6582_AP_BT_CVSD_IRQ_LINE, (irq_handler_t)AudDrv_BTCVSD_IRQ_handler, IRQF_TRIGGER_LOW/*                    */, "BTCVSD_ISR_Handle", dev);
    if (ret < 0)
    {
