...
#define K2DH_ACCEL_CALIBRATION        //                                                
/*                                                                            */
#define HIGH_RESOLUTION                 0x08
#define K2HH_HIGH_RESOLUTION                 0x80

#define AXISDATA_REG                        0x28
#define WHOAMI_K2DH_ACC                    0x33    /*                            */ 
#define K2HH_WHOAMI_K2DH_ACC              0x41 //        
...
#define INT_CFG2                                 0x34    /*                          */
#define INT_SRC2                                 0x35    /*                          */
#define INT_THS2                                 0x36    /*                             */
#define INT_DUR2                                 0x37    /*                            */
...
//        
#define K2HH_ODR_POWERDOWN_MODE           0x00  /*                 */
#define K2HH_ODR10              0x10  /*                       */
#define K2HH_ODR50              0x02  /*                       */
#define K2HH_ODR100             0x30  /*                        */
#define K2HH_ODR200             0x40  /*                        */  //              
#define K2HH_ODR400             0x50  /*                        */
#define K2HH_ODR800             0x60    /*                        */

#define ODR_POWERDOWN_MODE                0x00  /*                 */
#define ODR1                0x10  /*                      */
#define ODR10              0x20  /*                       */
#define ODR25              0x30  /*                       */
#define ODR50              0x40  /*                       */
...
#define K2DH_SHAKING_DETECT_THRESHOLD    (200)     //                     
...
#define TESTLIMIT_XY                  (130)  //     
#define TESTLIMIT_Z_USL_LSB           (635) //      
#define TESTLIMIT_Z_LSL_LSB           (390) //     
...
static struct i2c_board_info __initdata k2hh_i2c_info ={ I2C_BOARD_INFO(K2DH_ACC_DEV_NAME, K2HH_ACC_I2C_SAD_L)}; //        
    ...
    if(K2HH)
    	{
		    ...
		    {
		        res = k2dh_SetBWRate(client, K2HH_ODR_POWERDOWN_MODE);  //                
		    }
    	}
    else
    	{
		    ...
		    {
		        res = k2dh_SetBWRate(client, ODR_POWERDOWN_MODE);  //                
		    }
    	}
...
static int k2dh_acc_hw_init(struct i2c_client *client, int reset_cali)
{
   ...
   if(K2HH) //        
    ...
    //                                                
    if(K2HH){
    res = k2dh_SetDataFormat(client, K2HH_ACC_G_4G);//                                
	}
    else{
    res = k2dh_SetDataFormat(client, K2DH_ACC_G_4G);//                                
	}
    if(res != K2DH_SUCCESS)
    {
        return res;
    ...
    res = k2dh_SetIntEnable(client, 0x00);  //                  
    if(res != K2DH_SUCCESS)
    {
        return res;
    }
...
static int k2dh_ReadData(struct i2c_client *client, s16 data[K2DH_AXES_NUM])
{
    ...
    if(K2HH==0)
  {
   ...
   /*                            */
        ...
        {
           check_cnt++;
            if(check_cnt >= READ_DATA_CHECK_CNT)  //                    
            {
        ...
        if((prev_data[K2DH_AXIS_X] == data[K2DH_AXIS_X]) && (prev_data[K2DH_AXIS_Y] == data[K2DH_AXIS_Y]) && (prev_data[K2DH_AXIS_Z] == data[K2DH_AXIS_Z]))
        {
            ...
            if(check_cnt >= READ_DATA_CHECK_CNT)  //                    
            {
                GSE_ERR("READ DATA SAME CASE : (%d %d %d)\n", data[K2DH_AXIS_X], data[K2DH_AXIS_Y],data[K2DH_AXIS_Z]);
                //                                                                  
                k2dh_SetBWRate(client, ODR_POWERDOWN_MODE);  //                
                msleep(100);                
                k2dh_SetBWRate(client, ODR200);  //                                        
                check_cnt = 0;
            }
        }
        else
...
static int k2dh_do_calibration(void)
{
      ...
      /*                                    */
       msleep ( 300 ); 
        ...
        obj->cali_sw[K2DH_AXIS_X] = sum[K2DH_AXIS_X] / CALIBRATION_DATA_AMOUNT;  //                  
        obj->cali_sw[K2DH_AXIS_Y] = sum[K2DH_AXIS_Y] / CALIBRATION_DATA_AMOUNT;  //                  

        if(sum[K2DH_AXIS_Z] < 0)
        {
            obj->cali_sw[K2DH_AXIS_Z] = (sum[K2DH_AXIS_Z] / CALIBRATION_DATA_AMOUNT) + 512;    //                                                         
        }
        else
        {
            obj->cali_sw[K2DH_AXIS_Z] = (sum[K2DH_AXIS_Z] / CALIBRATION_DATA_AMOUNT) - 512;    //                                                          
...
{
    int selfCalibration = 1;  //     

    struct i2c_client *client = k2dh_i2c_client;
    struct k2dh_i2c_data *obj = i2c_get_clientdata(client);
	if(atomic_read(&obj->fast_calib_rslt) == 1)  //                    
    {
        selfCalibration = 0;  //        
    }
    else
...
static ssize_t k2dh_SelfTest_show(struct device_driver *ddri, char *buf)
{
    ...
    if(atomic_read(&obj->selftest_rslt) == 1)  //                 
    {
        selftest = 0;  //        
    }
    else
