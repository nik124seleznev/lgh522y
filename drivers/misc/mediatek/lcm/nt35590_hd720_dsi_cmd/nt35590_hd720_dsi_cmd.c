...
static void lcm_get_params(LCM_PARAMS *params)
{
		...
		params->dsi.HS_ZERO 	= NS_TO_CYCLE((105 + 6 * ui), cycle_time);//              
		params->dsi.HS_PRPR 	= NS_TO_CYCLE((40 + 4 * ui), cycle_time);//                           
		//                    
		if (params->dsi.HS_PRPR < 2)
			params->dsi.HS_PRPR = 2;

		params->dsi.LPX 		= NS_TO_CYCLE(50, cycle_time);//        
		
		params->dsi.TA_SACK 	= 1;
		params->dsi.TA_GET		= 5 * NS_TO_CYCLE(50, cycle_time);//     
		params->dsi.TA_SURE 	= 3 * NS_TO_CYCLE(50, cycle_time) / 2;//                   
		params->dsi.TA_GO		= 4 * NS_TO_CYCLE(50, cycle_time);//     
	
		params->dsi.CLK_TRAIL	= NS_TO_CYCLE(60, cycle_time);//        
		//                      
		if (params->dsi.CLK_TRAIL < 2)
			params->dsi.CLK_TRAIL = 2;
		params->dsi.CLK_ZERO	= NS_TO_CYCLE((300-38), cycle_time);//              
