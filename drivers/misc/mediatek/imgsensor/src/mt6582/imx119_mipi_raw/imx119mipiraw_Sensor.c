...
/*                                              */
kal_uint16 IMX119MIPI_MAX_EXPOSURE_LINES = IMX119MIPI_PV_FRAME_LENGTH_LINES-3;//    
...
#define IMX119MIPI_MaxGainIndex (87 + 1)																				 //           
//                                                                                       
...
void IMX119MIPISetFlipMirror(kal_int32 imgMirror)
{
    kal_uint8  iTemp; 
	#if 0
    iTemp = IMX119MIPI_read_cmos_sensor(0x0101) & 0x03;	//                               
    switch (imgMirror)
    {
        case IMAGE_NORMAL:
            IMX119MIPI_write_cmos_sensor(0x0101, 0x03);	//          
            break;
        case IMAGE_V_MIRROR:
            IMX119MIPI_write_cmos_sensor(0x0101, iTemp | 0x01);	//        
            break;
        case IMAGE_H_MIRROR:
            IMX119MIPI_write_cmos_sensor(0x0101, iTemp | 0x02);	//          
            break;
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    switch (ScenarioId)
    {
        ...
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
            ...
            pSensorInfo->SensorPixelClockCount= 3;
            pSensorInfo->SensorDataLatchCount= 2;
            pSensorInfo->SensorGrabStartX = IMX119MIPI_IMAGE_SENSOR_CAP_STARTX;	//                                     
            pSensorInfo->SensorGrabStartY = IMX119MIPI_IMAGE_SENSOR_CAP_STARTY;	//                                                 
            pSensorInfo->SensorMIPILaneNumber = SENSOR_MIPI_1_LANE;			
            pSensorInfo->MIPIDataLowPwr2HighSpeedTermDelayCount = 0; 
            pSensorInfo->MIPIDataLowPwr2HighSpeedSettleDelayCount = 14; 
            pSensorInfo->MIPICLKLowPwr2HighSpeedTermDelayCount = 0; 
...
UINT32 IMX119MIPISetVideoMode(UINT16 u2FrameRate)
{
	...
	SENSORDB("[IMX119MIPI]enter IMX119MIPISetVideoMode function u2FrameRate=%d\n",u2FrameRate);
		...
		if (IMX119MIPI_Video_Max_Expourse_Time > IMX119MIPI_PV_FRAME_LENGTH_LINES/*                                 */) 
			{
				spin_lock(&imx119_drv_lock);    
...
UINT32 IMX119MIPISetTestPatternMode(kal_bool bEnable)
{
    ...
    if(bEnable) {   //                    
        IMX119MIPI_write_cmos_sensor(0x30D8, 0x10);  //                       
        IMX119MIPI_write_cmos_sensor(0x0600, 0x00);  //                       
        IMX119MIPI_write_cmos_sensor(0x0601, 0x02);  //                        
    } else {
        IMX119MIPI_write_cmos_sensor(0x30D8, 0x00);  //                               
    }
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
        		switch(CurrentScenarioId_119)
        		{
        			case MSDK_SCENARIO_ID_CAMERA_ZSD:
        			case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		            *pFeatureReturnPara32 = IMX119MIPI_sensor.cp_pclk; //         
		            *pFeatureParaLen=4;		         	
		         		break;
		         		
		         		default:
		            *pFeatureReturnPara32 = IMX119MIPI_sensor.pv_pclk;//                     
		            *pFeatureParaLen=4;
		            break;
		         }
		         break;
