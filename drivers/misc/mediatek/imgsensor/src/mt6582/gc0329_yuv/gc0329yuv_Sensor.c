...
static void Gc0329InitialPara(void)
{
  ...
  Gc0329CurrentStatus.iFrameRate = 0;//                
...
void GC0329_Sensor_Init(void)
{
	...
	GC0329_write_cmos_sensor(0x83, 0x02);  //                        
	...
	GC0329_write_cmos_sensor(0x3e, 0x48); 
			...
			GC0329_SET_PAGE1;
			GC0329_write_cmos_sensor(0x29, 0x00);   //                        
			GC0329_write_cmos_sensor(0x2a, 0x3c);   //                       
			
			GC0329_write_cmos_sensor(0x2b, 0x02);   //                     
			GC0329_write_cmos_sensor(0x2c, 0x58); 
			GC0329_write_cmos_sensor(0x2d, 0x02);   //                     
			GC0329_write_cmos_sensor(0x2e, 0xd0); 
			GC0329_write_cmos_sensor(0x2f, 0x03);   //                     
			GC0329_write_cmos_sensor(0x30, 0xc0); 
			GC0329_write_cmos_sensor(0x31, 0x04);   //                    
	...
	GC0329_write_cmos_sensor(0x51, 0x18);//  
	GC0329_write_cmos_sensor(0x52, 0x0b);
	GC0329_write_cmos_sensor(0x53, 0x1d);//  
	GC0329_write_cmos_sensor(0x54, 0x35);//  
...
BOOL GC0329_set_param_wb(UINT16 para)
{
	...
	switch (para)
	{
		...
		case AWB_MODE_CLOUDY_DAYLIGHT: //      
			...
			GC0329_write_cmos_sensor(0x77, 0x8c); //               
...
BOOL GC0329_set_param_banding(UINT16 para)
{
	switch (para)
	{
		...
		case AE_FLICKER_MODE_60HZ:
			...
			GC0329_SET_PAGE1;
			GC0329_write_cmos_sensor(0x29, 0x00);   //                        
			GC0329_write_cmos_sensor(0x2a, 0x32);   //                       
