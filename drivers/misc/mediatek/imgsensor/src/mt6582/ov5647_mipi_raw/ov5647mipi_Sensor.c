...
#if defined(OV5647MIPI_USE_LENC_OTP)    //                         
...
UINT32 OV5647MIPISetMaxFrameRate(UINT16 u2FrameRate)
{
	...
	spin_unlock_irqrestore(&ov5647mipi_drv_lock,flags);
	    ...
	    /*                                 */
		OV5647MIPI_Set_Dummy(0, dummy_line); /*                                            */
	// 
	return ERROR_NONE;
}
...
/*                                  */
	...
	if (max_exposure_lines > OV5647MIPI_sensor.frame_height) /*                                                      */
//  
//                                                                    
//                                                 
//  
...
static void OV5647MIPI_camera_para_to_sensor(void)
{
  ...
  OV5647MIPI_SetGain(OV5647MIPI_sensor.gain); /*             */
}
...
static void OV5647MIPI_Sensor_Init(void)
{
	...
	OV5647MIPI_write_cmos_sensor(0x3035, 0x11); //                
	OV5647MIPI_write_cmos_sensor(0x303c, 0x11); //         
	OV5647MIPI_write_cmos_sensor(0x370c, 0x0f); //                                             
	...
	OV5647MIPI_write_cmos_sensor(0x3002, 0x00); //      
	...
	OV5647MIPI_write_cmos_sensor(0x5000, 0x06);
	OV5647MIPI_write_cmos_sensor(0x4800, 0x14);//               
	OV5647MIPI_write_cmos_sensor(0x4001, 0x02);		 //            
	OV5647MIPI_write_cmos_sensor(0x4000, 0x09);		//            
	//                                                     

	OV5647MIPI_write_cmos_sensor(0x3013,0x00);//
	OV5647MIPI_write_cmos_sensor(0x4005,0x18);//           
	OV5647MIPI_write_cmos_sensor(0x4050,0x37);//       
...
static void OV5647MIPI_Sensor_1080P(void) 
{
	...
	//                                                       
	OV5647MIPI_write_cmos_sensor(0x4005,0x1a);//           
