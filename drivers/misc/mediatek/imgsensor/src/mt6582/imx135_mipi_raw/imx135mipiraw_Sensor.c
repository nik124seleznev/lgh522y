...
kal_uint32 capture_gain_imx135 = 64;																				 //           
...
/*                                      
	...
	//                      
	//                                          
	IMX135MIPI_write_cmos_sensor(0x205, iReg&0xFF);//                                
	//                                          
...
void update_capture_shutter_gain_imx135()
{
	...
	IMX135MIPI_SetGain_capture(capture_gain_imx135); //                                         
...
//       
   ...
   //                       
    ...
    IMX135MIPI_sensor.is_zsd = KAL_FALSE;  //                         
...
{	
 ...
 //            
	...
	IMX135MIPI_write_cmos_sensor(0x0341,0x68);//     
	...
	IMX135MIPI_write_cmos_sensor(0x0100,0x01);//             
...
{	
	...
	IMX135MIPI_write_cmos_sensor(0x0342,0x15);//          
	IMX135MIPI_write_cmos_sensor(0x0343,0x50);//        
	IMX135MIPI_write_cmos_sensor(0x0344,0x00);//
	IMX135MIPI_write_cmos_sensor(0x0345,0x00);//
	IMX135MIPI_write_cmos_sensor(0x0346,0x00);//
	IMX135MIPI_write_cmos_sensor(0x0347,0x00);//
...
#endif	
#if fast_mode
  IMX135MIPI_write_cmos_sensor(0x3A43,0x00);//               
#endif  
	IMX135MIPI_write_cmos_sensor(0x0100,0x01);//            
...
void IMX135MIPI_NightMode(kal_bool bEnable)
{
    ...
    else//                           
    {
        if(OV5642_MPEG4_encode_mode==KAL_TRUE)
        {
            ...
            if(OV5642_pv_exposure_lines < (OV5642_MAX_EXPOSURE_LINES - OV5642_SHUTTER_LINES_GAP)) //                                                                                     
            {
                OV5642_write_cmos_sensor(0x350C, (OV5642_MAX_EXPOSURE_LINES >> 8) & 0xFF);
                OV5642_write_cmos_sensor(0x350D, OV5642_MAX_EXPOSURE_LINES & 0xFF);
                OV5642_CURRENT_FRAME_LINES = OV5642_MAX_EXPOSURE_LINES;
...
void IMX135MIPISetFlipMirror(kal_int32 imgMirror)
{
    ...
    iTemp = IMX135MIPI_read_cmos_sensor(0x0101) & 0xFB;	//                               
    switch (imgMirror)
    {
        case IMAGE_NORMAL:
            //                                                         
            IMX135MIPI_write_cmos_sensor(0x0101, iTemp | 0x03);	//                   
            break;
        case IMAGE_V_MIRROR:
			IMX135MIPI_write_cmos_sensor(0x0101, iTemp | 0x02); //        
            break;
        case IMAGE_H_MIRROR:
			IMX135MIPI_write_cmos_sensor(0x0101, iTemp | 0x01); //          
            break;
        case IMAGE_HV_MIRROR:
            //                                                                         
            IMX135MIPI_write_cmos_sensor(0x0101, iTemp);	//          
            break;
    }
}
    ...
    switch (ScenarioId)
    {
     ...
     //                                             
       ...
       //                                          
            ...
            pSensorInfo->SensorPixelClockCount= 3;
            pSensorInfo->SensorDataLatchCount= 2;
            pSensorInfo->SensorGrabStartX = IMX135MIPI_IMAGE_SENSOR_CAP_STARTX;	//                                     
            pSensorInfo->SensorGrabStartY = IMX135MIPI_IMAGE_SENSOR_CAP_STARTY;	//                                                 
            pSensorInfo->SensorMIPILaneNumber = SENSOR_MIPI_4_LANE;			
            pSensorInfo->MIPIDataLowPwr2HighSpeedTermDelayCount = 0; 
            pSensorInfo->MIPIDataLowPwr2HighSpeedSettleDelayCount = 14; 
            pSensorInfo->MIPICLKLowPwr2HighSpeedTermDelayCount = 0; 
...
UINT32 IMX135MIPISetVideoMode(UINT16 u2FrameRate)
{
     ...
     if(u2FrameRate==0)
	{
		...
		if (IMX135MIPI_Video_Max_Expourse_Time > IMX135MIPI_VIDEO_FRAME_LENGTH_LINES/*                                 */) 
			{
				spin_lock(&imx135_drv_lock);    
...
if(IMX135MIPI_sensor.pv_mode==TRUE)
    ...
    if(bEnable) {   //                       
        ...
        IMX135MIPI_Auto_Flicker_mode = KAL_TRUE; 
		spin_unlock(&imx135_drv_lock);
        if(IMX135MIPI_MPEG4_encode_mode == KAL_TRUE) {    //                                        
...
UINT32 IMX135MIPISetTestPatternMode(kal_bool bEnable)  //      
{
    ...
    if(bEnable) {   //                    
        IMX135MIPI_write_cmos_sensor(0x30D8, 0x10);  //                       
        IMX135MIPI_write_cmos_sensor(0x0600, 0x00);  //                       
        IMX135MIPI_write_cmos_sensor(0x0601, 0x02);  //                        
    } else {
        IMX135MIPI_write_cmos_sensor(0x30D8, 0x00);  //                               
    }
    return ERROR_NONE;
}
