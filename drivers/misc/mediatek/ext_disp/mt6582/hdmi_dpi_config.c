...
void hdmi_dpi_config_update(int hdmi_width, int hdmi_height)
{
    ...
    DPI_CHECK_RET(HDMI_DPI(_ConfigDataEnable)(de_pol)); //              
    ...
    DPI_CHECK_RET(HDMI_DPI(_FBSetPitch)(DPI_FB_0, hdmi_width * 3)); //           
    DPI_CHECK_RET(HDMI_DPI(_FBEnable)(DPI_FB_0, TRUE)); //           

    //                           
    DPI_CHECK_RET(HDMI_DPI(_FBSetFormat)(DPI_FB_FORMAT_RGB888)); //           

    if (HDMI_COLOR_ORDER_BGR == rgb_order)
    {
        DPI_CHECK_RET(HDMI_DPI(_SetRGBOrder)(DPI_RGB_ORDER_RGB, DPI_RGB_ORDER_BGR)); //           
    }
    else
    {
        DPI_CHECK_RET(HDMI_DPI(_SetRGBOrder)(DPI_RGB_ORDER_RGB, DPI_RGB_ORDER_RGB)); //           
    }
    //                               
    //                                               
}
...
void hdmi_dpi_setting_res(u8 arg, HDMI_CABLE_TYPE cable_type, _t_hdmi_context *hdmi_context_unstatic)
{
    switch (arg)
    {
        ...
        case HDMI_VIDEO_1280x720p_60Hz:
        {
            ...
            hdmi_context_unstatic->hdmi_width = 1280 - hdmi_context_unstatic->bg_width; //          
            hdmi_context_unstatic->hdmi_height = 720 - hdmi_context_unstatic->bg_height;//         
