...
static struct wag_init_info* wake_gesture_init= {0}; //        
...
static int wag_real_enable(int enable)
{
	...
	if(1==enable)
	{
		...
		if(atomic_read(&(wag_context_obj->early_suspend))) //                                 
		{
			return 0;
		}
...
static int wag_real_driver_init(void) 
{
	...
	wag_context_obj->notify_timer.expires	= HZ/5; //      
...
static void wag_early_suspend(struct early_suspend *h) 
{
	...
	if(!atomic_read(&wag_context_obj->wake)) //                                     
	{
