...
static GFH_CHECK_CFG_v1        dsp_sys_head[MAX_MD_NUM]; //                  
...
void config_misc_info(int md_id, unsigned int base[], unsigned int size)
{
    ...
    if(NULL != base) {
		...
		else
		{
			...
			memcpy((char*)&(misc_info.feature_4_val[3]), temp_buf_svn, 4); /*     */
		...
		else
		{
			...
			memcpy((char*)&(misc_info.feature_4_val[0]), mcc, 4); /*     */
			memcpy((char*)&(misc_info.feature_4_val[1]), mnc, 4); /*     */
			memcpy((char*)&(misc_info.feature_4_val[2]), subset, 4); /*        */
...
static void enable_mem_access_protection(int md_id)
{
    ...
    rom_mem_phy_end   = (md_layout->md_region_phy + img_info[MD_INDEX].size + 0xFFFF)&(~0xFFFF); //          
...
static unsigned int INVALID_ADDR[MAX_MD_NUM];//            
