...
static struct inpk_init_info* in_pocket_init= {0}; //        
...
static int inpk_real_enable(int enable)
{
	...
	if(1==enable)
	{
		...
		if(atomic_read(&(inpk_context_obj->early_suspend))) //                                 
		{
			return 0;
		}
...
static void inpk_early_suspend(struct early_suspend *h) 
{
	...
	if(!atomic_read(&inpk_context_obj->wake)) //                                     
	{
...
{
	atomic_set(&(inpk_context_obj->early_suspend), 0);
	if(!atomic_read(&inpk_context_obj->wake) && resume_enable_status) //                                     
	{
