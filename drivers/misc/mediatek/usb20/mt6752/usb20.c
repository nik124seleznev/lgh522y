...
static void musb_do_idle(unsigned long _musb)
{
	...
	unsigned long	flags;
	u8	devctl, repeat_devctl1, repeat_devctl2;

	printk("before otg_state %s power: 0x%x, is_active: 0x%x\n", otg_state_string(musb->xceiv->state), musb->power, musb->is_active);
	...
	case OTG_STATE_A_WAIT_BCON:
		...
		repeat_devctl1 = musb_readb(musb->mregs, MUSB_DEVCTL);
		if ((devctl == 0) || (devctl & MUSB_DEVCTL_BDEVICE)) {
	...
	spin_unlock_irqrestore(&musb->lock, flags);
	repeat_devctl2 = musb_readb(musb->mregs, MUSB_DEVCTL);
	//                                                                                  
	printk("after otg_state %s devctlx: 0x%x, 0x%x devctl: 0x%x\n", otg_state_string(musb->xceiv->state), repeat_devctl1, repeat_devctl2, devctl);
}
