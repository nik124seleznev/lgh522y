...
static volatile bool   AudDrvSuspendStatus            = false; //                
static bool   AudIrqReset                              = false; //                       
...
extern int        Aud_AFE_Clk_cntr;
int  Aud_Ext_Mem_Flag = 0; //                                                                                                                
int  Aud_Int_Mem_Flag = 0; //                                                                                                                
...
{
    WriteRecordArray[WriteArrayIndex] = t2; //      
    WriteArrayIndex++;
...
static int AudDrv_probe(struct platform_device *dev)
{
    ...
    ret = request_irq(HW_AFE_MCU_IRQ_LINE, AudDrv_IRQ_handler, IRQF_TRIGGER_LOW/*                    */, "Afe_ISR_Handle", dev);
    if (ret < 0)
    {
...
static int AudDrv_resume(struct platform_device *dev) //        
{
    //                                                                              
    if (AudDrvSuspendStatus == true)
    {
...
int AudDrv_Force_Free_Buffer(int mem_type)
{
    ...
    switch (mem_type)
    {
        ...
        case MEM_DAI:
        {
            ...
            pblock =  &(pAFE_MEM->rBlock);
#if defined(MTK_AUDIO_DYNAMIC_SRAM_SUPPORT)
            if (Aud_Int_Mem_Flag & (1 << MEM_DAI) == 0) //                       
#endif
            Auddrv_Free_Dma_Memory(pAFE_MEM);
            memset((void *)&DAI_Control_context, 0, sizeof(AFE_MEM_CONTROL_T));
            DAI_Control_context.MemIfNum = MEM_DAI ;
...
#if defined(MTK_AUDIO_DYNAMIC_SRAM_SUPPORT)
    spin_lock(&auddrv_lock);
    if ((Aud_Int_Mem_Flag & (1 << ((int)arg))) != 0) //                                               
    {
