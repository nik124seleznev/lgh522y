...
static wakeup_ctl_info_t	wakeup_user[WAKEUP_EMD_USER_NUM]; //              
...
static struct wake_lock		emd_wdt_wake_lock;	//                                   
...
/*                      */
#define EMD_DBG_NONE		(0x00000000)    /*              */
#define EMD_DBG_FUNC		(0x00000001)    /*                    */
...
/*                                                                                         
                                                                                          ...
                                                                                          */ 
static int request_ext_md_reset(void); //                     
		...
		if( (wakeup_user[i].dev_major_id == dev_id)
		  &&(wakeup_user[i].dev_sub_id == dev_sub_id) ){
		 	if(wakeup_user[i].manual_req_count == 0){
				...
				if(0 == timeout_pending_count){
					timeout_pending_count++;
					mod_timer(&emd_wakeup_timer, jiffies+HZ); //                    
				}
				spin_unlock_irqrestore(&wakeup_ctl_lock, flags);
			}
			break;
