...
static inline int DSI_video_TE_eint_setup(void)
{
    ...
    mt_set_gpio_pull_enable(GPIO_DSI_TE_PIN, GPIO_PULL_DISABLE); //                     
...
static unsigned int DSI_Detect_CLK_Glitch_Default(void)
{
	...
	OUTREG32(&DSI_CMDQ_REG->data[0], 0x00340500);//           
	OUTREG32(&DSI_REG->DSI_CMDQ_SIZE, 1);
			...
			break;
		 ...
		 else
			 {
			  ...
			  if(glitch_log_on)
                  ...
                  if(((read_data0.byte1&0x4) != 0)||((read_data0.byte2&0x3)!=0)) //                
                    {
...
//                
	...
	OUTREGBIT(MIPITX_DSI0_CLOCK_LANE_REG,DSI_PHY_REG->MIPITX_DSI0_CLOCK_LANE,RG_DSI0_LNTC_PHI_SEL,0x1);
        #if defined(TARGET_MT6582_Y50)  /*                                                                        */
        OUTREGBIT(MIPITX_DSI0_CLOCK_LANE_REG,DSI_PHY_REG->MIPITX_DSI0_CLOCK_LANE,RG_DSI0_LNTC_LPTX_IPLUS1,0x1); //           
        OUTREGBIT(MIPITX_DSI0_CLOCK_LANE_REG,DSI_PHY_REG->MIPITX_DSI0_CLOCK_LANE,RG_DSI0_LNTC_LPTX_IPLUS2,0x0); //                               
        //                                                                                                                          
        #endif  /*                                                                        */
	...
	if(lcm_params->dsi.LANE_NUM > 0)
	{
        ...
        #if defined(TARGET_MT6582_Y50)  /*                                                                        */
        OUTREGBIT(MIPITX_DSI0_DATA_LANE0_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE0,RG_DSI0_LNT0_LPTX_IPLUS1,0x1); //           
        OUTREGBIT(MIPITX_DSI0_DATA_LANE0_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE0,RG_DSI0_LNT0_LPTX_IPLUS2,0x0); //                               
        //                                                                                                                              
        #endif  /*                                                                        */
	...
	if(lcm_params->dsi.LANE_NUM > 1)
	{
        ...
        #if defined(TARGET_MT6582_Y50)  /*                                                                        */
        OUTREGBIT(MIPITX_DSI0_DATA_LANE1_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE1,RG_DSI0_LNT1_LPTX_IPLUS1,0x1); //           
        OUTREGBIT(MIPITX_DSI0_DATA_LANE1_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE1,RG_DSI0_LNT1_LPTX_IPLUS2,0x0); //                               
        //                                                                                                                                
        #endif  /*                                                                        */
...
{
	printk("BLS: 0x%x\n", INREG32(0xF400A000));
	return (((INREG32(0xF400A000)) & (0x1)) == 0x1);//                
}
...
DSI_STATUS DSI_enable_MIPI_txio(bool en)
{
    ...
    if(en)
    {
        *(volatile unsigned int *) (INFRACFG_BASE+0x890) |= 0x00000100;    //                  
    }
    else
    {
        *(volatile unsigned int *) (INFRACFG_BASE+0x890) &= ~0x00000100;   //                   
