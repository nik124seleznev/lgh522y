...
void  DPI_MIPI_clk_setting(unsigned int mipi_pll_clk_ref,unsigned int mipi_pll_clk_div1,unsigned int mipi_pll_clk_div2){
	...
	UINT32 sdm_ssc_en     = 0;
	UINT32 sdm_ssc_prd    = 0;  //        
	UINT32 sdm_ssc_delta1 = 0;  //        
	UINT32 sdm_ssc_delta  = 0;  //          
...
DPI_MIPI_clk_setting( mipi_pll_clk_ref,  mipi_pll_clk_div1, mipi_pll_clk_div2);
	...
	MASKREG16(0xc20a380c, 0x000c, 0x0000); //                                             
	#else
	OUTREG16(0xf2080858, 0x8000); //  
	OUTREG16(0xf20a3824, 0x4008);
	MASKREG16(0xf20a380c, 0x000c, 0x0000); //                                             
...
//                   
#if 0
	OUTREG32(DISPSYS_BASE + 0x34, 0x02);//                
#if 0
	//  
	OUTREG32(DISPSYS_BASE + 0x104, 0xffffffff);//      
	OUTREG32(DISPSYS_BASE + 0x108, 0xffffffff);
	OUTREG32(DISPSYS_BASE + 0x114, 0xffffffff);
	OUTREG32(DISPSYS_BASE + 0x118, 0xffffffff);
#endif
	OUTREG32(DISP_MUTEX_BASE + 0x28, 0x01);//           
	OUTREG32(DISP_MUTEX_BASE + 0x28, 0);
	
	OUTREG32(DISP_MUTEX_BASE + 0x2c, 0x80); //                     
	OUTREG32(DISP_MUTEX_BASE + 0x30, 0x2);  //               

	OUTREG32(DISP_MUTEX_BASE + 0x24, 0x1);  //           
	
	OUTREG32(DISP_MUTEX_BASE + 0, 0x1);  //           
	//                                                

	while((INREG32(DISP_MUTEX_BASE + 0x24)&0x02)!=0x02){} //                                  
	
	OUTREG32(DISPSYS_BASE + 0xC08, 0x01);//              
	OUTREG32(DISPSYS_BASE + 0x60, 0x8);//              
	
	//             
	OUTREG32(RDMA0_BASE + 0x10, 0); //          
	
	//                         
	OUTREG32(RDMA0_BASE + 0x14, lcm_params->width);
	OUTREG32(RDMA0_BASE + 0x18, lcm_params->height);
	
	OUTREG32(RDMA0_BASE + 0x24, 0x40);//                    
#ifndef BUILD_UBOOT
	OUTREG32(RDMA0_BASE + 0x28, FB_Addr);//          
#else
	OUTREG32(RDMA0_BASE + 0x28, mt65xx_get_fb_addr());//          
#endif
	OUTREG32(RDMA0_BASE + 0x2C, lcm_params->width*2);//           
...
DPI_STATUS DPI_PowerOff()
{
    if (s_isDpiPowerOn)
    {
        ...
        _BackupDPIRegisters();
#if 1   //      
        MASKREG32(0x14000110, 0x40, 0x40);//                       
#endif
        ASSERT(ret);
