...
#define MTK_FB_YRESV (ALIGN_TO(MTK_FB_YRES, disphal_get_fb_alignment()) * MTK_FB_PAGES) /*                   */
...
int mtkfb_display_kcal(int is_white)
{
	...
	if(is_white)
	{
		...
		mipi_fb_cfg.stride = ALIGN_TO(mipi_fb_cfg.width, MTK_FB_ALIGNMENT);
                ...
                memset(mipi_fb_cfg.base, 0xffffffff, mipi_fb_cfg.stride * mipi_fb_cfg.height * ((mipi_fb_cfg.bpp) / 8) * 3);  //                 
...
static int mtkfb_capture_framebuffer(struct fb_info *info, unsigned int pvbuf)
{
    ...
    */
    ...
    if (atomic_read(&capture_ui_layer_only))
    {
        ...
        unsigned int fbaddress = info->fix.smem_start + info->var.yoffset * info->fix.line_length; //                
    ...
    case MTKFB_PREPARE_OVERLAY_BUFFER:
    {
        ...
        else 
        {
            if (overlay_buffer.layer_en) 
            {
                ...
                {
                    overlay_buffer.fence_fd = DISP_INVALID_FENCE_FD; //           
	...
	case DISP_IOCTL_PREPARE_INPUT_BUFFER:
    {
        ...
        if (info.layer_en) {
            if (disp_sync_prepare_buffer(&info) != SYNC_STATUS_OK) {
                info.fence_fd = DISP_INVALID_FENCE_FD; //           
                info.index = 0;
                r = -EFAULT;
            }
        } else {
...
static void mtkfb_fb_8888_to_8888(struct fb_info *fb_info)
{
    ...
    unsigned int fbsize = x_virtual * yres * 4; //                   
...
static void mtkfb_fb_8888_to_888(void* fb_va)
{
    ...
    for (j = 0; j < yres; ++j)
    {
        s = (unsigned int *)(fb_va + j * ALIGN_TO(xres, MTK_FB_ALIGNMENT) * 4); //                      
        d = (char *)(fb_va + DISP_GetFBRamSize() + DAL_GetLayerSize() + j * xres * 3); //                    
        for (k = 0; k < xres; ++k)
        {
            src_argb8888 = *s++;
            *d++ = ((src_argb8888 & 0xFF));
