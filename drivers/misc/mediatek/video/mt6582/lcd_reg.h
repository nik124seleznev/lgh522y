...
typedef struct
{
    ...
    LCD_REG_INTERRUPT         	INT_ENABLE;         //     
    LCD_REG_INTERRUPT         	INT_STATUS;         //     
    LCD_REG_START             	START;              //     
    UINT32                    	RESET;              //     
    UINT32                    	rsv_0014[2];        //           
	LCD_REG_SIF_TIMING		  	SIF_TIMING[2];	  	//           
	UINT32                    	rsv_0024;           //     
	LCD_REG_SCNF			  	SERIAL_CFG;		  	//     
	LCD_REG_SIF_CS			  	SIF_CS;			  	//     
    LCD_REG_PCNF              	PARALLEL_CFG[3];    //           
    LCD_REG_PCNFDW            	PARALLEL_DW;        //     
    LCD_REG_TECON             	TEARING_CFG;        //     
	LCD_REG_CALC_HTT			CALC_HTT;			//     
	LCD_REG_SYNC_LCM_SIZE		SYNC_LCM_SIZE;		//     
	LCD_REG_SYNC_CNT			SYNC_CNT;			//     
    LCD_REG_SMICON            	SMI_CON;            //     
    UINT32                    	rsv_0054[3];        //           
    LCD_REG_WROI_CON          	WROI_CONTROL;       //     
    LCD_REG_CMD_ADDR           	WROI_CMD_ADDR;      //     
    LCD_REG_DAT_ADDR           	WROI_DATA_ADDR;     //     
    LCD_REG_SIZE              	WROI_SIZE;          //     
    LCD_SRC_CON                 SRC_CON;            //     
    UINT32                      SRC_ADD;            //     
    UINT32                      SRC_PITCH;          //     
    UINT32                    	rsv_007C[5];           //           
//                                                          
//                                                                
    LCD_REG_ULTRA_CON          	ULTRA_CON;          //     
    UINT32                    	CONSUME_RATE;       //     
    LCD_REG_DBI_ULTRA_TH       	DBI_ULTRA_TH;       //     
    LCD_REG_GMC_ULTRA_TH       	GMC_ULTRA_TH;       //     
    UINT32         	            rsv_00A0[728];      //           
	UINT32						CMDQ[32];		    //           
	UINT32						rsv_1D00[160];		//           
    UINT32                    	PCMD0;              //     
