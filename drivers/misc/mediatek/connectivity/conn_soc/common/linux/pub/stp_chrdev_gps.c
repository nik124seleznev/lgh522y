...
static int GPS_devs = 1;        /*              */
static int GPS_major = GPS_DEV_MAJOR;       /*                    */
module_param(GPS_major, uint, 0);
static struct cdev GPS_cdev;

static unsigned char i_buf[MTKSTP_BUFFER_SIZE];    //                       
static unsigned char o_buf[MTKSTP_BUFFER_SIZE];    //                         
...
long GPS_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case COMBO_IOC_GPS_HWVER:             
            ...
            GPS_INFO_FUNC(KERN_INFO "GPS_ioctl(): get hw version = %d, sizeof(hw_ver_sym) = %zd\n", hw_ver_sym, sizeof(hw_ver_sym));
