...
static const WMT_IC_INFO_S mt6620_info_table[] = {
	 ...
	 },
	{
	 ...
	 .bWorkWithoutPatch = MTK_WCN_BOOL_TRUE /*                   */ ,
	 .bPsmSupport = MTK_WCN_BOOL_TRUE,
	 },
	{
	 .u4HwVer = 0x8B30,
...
static INT32 mt6620_sw_init(P_WMT_HIF_CONF pWmtHifConf)
{
	...
	if (WMT_HIF_UART == pWmtHifConf->hifType) {
		...
		if (WMT_UART_MTK_SW_FC == pWmtHifConf->uartFcCtrl) {
			...
			WMT_SET_BAUD_CMD_X[8] = (UINT8) 0x80;	/*                     */
			WMT_QUERY_BAUD_EVT_X[9] = (UINT8) 0x80;	/*                     */
			...
			WMT_SET_BAUD_CMD_X[8] = (UINT8) 0xC0;	/*                 */
			WMT_QUERY_BAUD_EVT_X[9] = (UINT8) 0xC0;	/*                 */
		} else {
			/*                                       */
...
static const WMT_IC_INFO_S *mt6620_find_wmt_ic_info(const UINT32 hw_ver)
{
	...
	/*            */
	while ((0 <= index)
	       && (hw_ver != mt6620_info_table[index].u4HwVer)	/*            */
	    ) {
		--index;
	}
	if (0 <= index) {
