...
static const WMT_IC_INFO_S *mt6628_find_wmt_ic_info(const UINT32 hw_ver)
{
	...
	/*            */
	while ((0 <= index)
	       && (hw_ver != mt6628_info_table[index].u4HwVer)	/*            */
	    ) {
		--index;
	}
	if (0 <= index) {
...
static INT32 mt6628_set_sdio_driving(VOID)
{
	...
	ret =
	    wmt_core_init_script(sdio_driving_table, ARRAY_SIZE(sdio_driving_table));
