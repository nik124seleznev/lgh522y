...
/*                                                                   */
static UINT8 WMT_SET_REG_CMD[] = { 0x01, 0x08, 0x10, 0x00	/*       */
	    , 0x00		/*                */
	    , 0x01		/*          */
	    , 0x00		/*    */
	    , 0x01		/*           */
...
static UINT8 WMT_SET_REG_WR_EVT[] = { 0x02, 0x08, 0x04, 0x00	/*       */
	    , 0x00		/*     */
	    , 0x00		/*          */
	    , 0x00		/*    */
	    , 0x01		/*           */
...
static UINT8 WMT_SET_REG_RD_EVT[] = { 0x02, 0x08, 0x04, 0x00	/*       */
	    , 0x00		/*     */
	    , 0x00		/*          */
	    , 0x00		/*    */
	    , 0x01		/*           */
...
static UINT8 WMT_ANT_RAM_STA_GET_EVT[] = { 0x02, 0x06, 0x03, 0x00	/*       */
	    , 0x05, 0x02, 0x00	/*          */
};
...
static UINT8 WMT_ANT_RAM_DWN_EVT[] = { 0x02, 0x15, 0x01, 0x00	/*       */
	, 0x00
};
...
INT32 wmt_core_func_ctrl_cmd(ENUM_WMTDRV_TYPE_T type, MTK_WCN_BOOL fgEn)
{
	...
	rWmtPktCmd.u2SduLen = WMT_FLAG_LEN + WMT_FUNC_CTRL_PARAM_LEN;	/*     */

	/*                      */
	u4WmtCmdPduLen = WMT_HDR_LEN + rWmtPktCmd.u2SduLen;	/*     */
	u4WmtEventPduLen = WMT_HDR_LEN + WMT_STS_LEN;	/*     */
...
INT32 wmt_core_opid_handler(P_WMT_OP pWmtOp)
{
	...
	if (wmt_core_opfunc[opId]) {
		ret = (*(wmt_core_opfunc[opId])) (pWmtOp);	/*                                   */
		return ret;
	} else {
...
INT32 wmt_core_reg_rw_raw(UINT32 isWrite, UINT32 offset, PUINT32 pVal, UINT32 mask)
{
	...
	WMT_SET_REG_CMD[4] = (isWrite) ? 0x1 : 0x2;	/*          */
	osal_memcpy(&WMT_SET_REG_CMD[8], &offset, 4);	/*        */
	osal_memcpy(&WMT_SET_REG_CMD[12], pVal, 4);	/*                 */
	osal_memcpy(&WMT_SET_REG_CMD[16], &mask, 4);	/*      */
...
static INT32 wmt_core_hw_check(VOID)
{
	...
	iret = wmt_core_reg_rw_raw(0, GEN_HCR, &chipid, GEN_HCR_MASK);	/*                 */
	if (iret) {
...
static INT32 opfunc_lpbk(P_WMT_OP pWmtOp)
{
	...
	/*                                 */
	MTK_WCN_BOOL fgFail;
	buf_length = pWmtOp->au4OpData[0];	/*               */
	pbuffer = (PUINT32) pWmtOp->au4OpData[1];	/*                       */
...
static INT32 opfunc_therm_ctrl(P_WMT_OP pWmtOp)
{
	...
	WMT_THERM_CMD[4] = pWmtOp->au4OpData[0];	/*                                        */
	...
	if (u4Res == osal_sizeof(WMT_THERM_CTRL_EVT)) {
		if (osal_memcmp(evtBuf, WMT_THERM_CTRL_EVT, osal_sizeof(WMT_THERM_CTRL_EVT)) != 0) {
			...
			pWmtOp->au4OpData[1] = MTK_WCN_BOOL_FALSE;	/*                               */
			mtk_wcn_stp_dbg_dump_package();
		} else {
			WMT_INFO_FUNC("Send WMT_THERM_CTRL_CMD command OK!\n");
			pWmtOp->au4OpData[1] = MTK_WCN_BOOL_TRUE;	/*                               */
		}
	} else {
		/*                                        */
		if (osal_memcmp(evtBuf, WMT_THERM_READ_EVT, osal_sizeof(WMT_THERM_READ_EVT) - 1) !=
			...
			pWmtOp->au4OpData[1] = 0xFF;	/*                               */
			mtk_wcn_stp_dbg_dump_package();
		} else {
			WMT_INFO_FUNC("Send WMT_THERM_READ_CMD command OK!\n");
			pWmtOp->au4OpData[1] = evtBuf[5];	/*                               */
		}
	}
...
static INT32 opfunc_efuse_rw(P_WMT_OP pWmtOp)
{
	...
	WMT_EFUSE_CMD[4] = (pWmtOp->au4OpData[0]) ? 0x1 : 0x2;	/*          */
	osal_memcpy(&WMT_EFUSE_CMD[6], (PUINT8) &pWmtOp->au4OpData[1], 2);	/*         */
	osal_memcpy(&WMT_EFUSE_CMD[8], (PUINT32) pWmtOp->au4OpData[2], 4);	/*       */
