...
SD_DEBUG_INSTANTIATE_ZONES(TEXT("SDIO NDIS Sample"),	/*              */
			   (ZONE_ENABLE_INIT | ZONE_ENABLE_INFO | ZONE_ENABLE_ERROR | ZONE_ENABLE_WARN | ENABLE_ZONE_SEND | ENABLE_ZONE_SPECIAL),	/*                   */
			   TEXT(""),
			   TEXT(""),
			   TEXT(""),
			   TEXT(""),
		  ...
		  IN ULONG u4BlockCount, IN ULONG u4BlockSize, IN PUCHAR pBuff, IN DWORD dwFlags)
{
	...
	/*                                                                                      */
	/*                                       */
	rRequest.TransferClass = tClass;	/*                */
	rRequest.CommandCode = ucCmd;	/*              */
	rRequest.CommandArgument = dwArg;	/*                  */
	rRequest.CommandResponse.ResponseType = rType;	/*                  */
	/*                                                                 */
	/*                                                 */
	rRequest.NumBlocks = u4BlockCount;	/*                  */
	rRequest.BlockSize = u4BlockSize;	/*                    */
	/*                                                                                    */
	rRequest.pBlockBuffer = pBuff;	/*                           */
	rRequest.pCallback = NULL;	/*                                     */
	/*                                                                                                               */
	rRequest.Flags = dwFlags;	/*               */
	/*                           */
	/*                        */
...
BOOL sdioCisTest(P_GL_HIF_INFO_T pDevice)
{
	...
	ULONG length;		/*              */
	SD_API_STATUS status;	/*        */
	UCHAR manfID[SD_CISTPL_MANFID_BODY_SIZE];	/*                 */
	UCHAR custom[CUSTOM_TUPLE_BODY];	/*                   */
...
#if CFG_SDIO_PATHRU_MODE
	status = SD_SYNC_BUS_REQ(prSdInfo, SD_CMD_IO_RW_EXTENDED, argument, SD_READ, ResponseR5, &response, 1,	/*              */
				 4,	/*            */
				 (PUCHAR) pu4Value, 0);
#else
	status = SDSynchronousBusRequest(prSdInfo->hDevice, SD_CMD_IO_RW_EXTENDED, argument, SD_READ, ResponseR5, &response, 1,	/*              */
					 4,	/*            */
					 (PUCHAR) pu4Value, 0);
#endif
...
#if CFG_SDIO_PATHRU_MODE
	status = SD_SYNC_BUS_REQ(prSdInfo, SD_CMD_IO_RW_EXTENDED, argument, SD_WRITE, ResponseR5, &response, 1,	/*              */
				 4,	/*            */
				 (PUCHAR) & u4Value, 0);
#else
	status = SDSynchronousBusRequest(prSdInfo->hDevice, SD_CMD_IO_RW_EXTENDED, argument, SD_WRITE, ResponseR5, &response, 1,	/*              */
					 4,	/*            */
					 (PUCHAR) & u4Value, 0);
#endif
	return SD_API_SUCCESS(status) ? TRUE : FALSE;
...
#if CFG_SDIO_PATHRU_MODE
		status = SD_SYNC_BUS_REQ(prSdInfo, SD_CMD_IO_RW_EXTENDED, argument, SD_READ, ResponseR5, &response, u4BlockNum,	/*              */
					 prSdInfo->sdHostBlockCap.ReadBlockSize,	/*            */
					 pucBuf, 0);
#else
		status = SDSynchronousBusRequest(prSdInfo->hDevice, SD_CMD_IO_RW_EXTENDED, argument, SD_READ, ResponseR5, &response, u4BlockNum,	/*              */
						 prSdInfo->sdHostBlockCap.ReadBlockSize,	/*            */
						 pucBuf, 0);
#endif
...
#if CFG_SDIO_PATHRU_MODE
		status = SD_SYNC_BUS_REQ(prSdInfo, SD_CMD_IO_RW_EXTENDED, argument, SD_WRITE, ResponseR5, &response, u4BlockNum,	/*              */
					 prSdInfo->sdHostBlockCap.WriteBlockSize,	/*            */
					 pucBuf, 0);
#else
		status = SDSynchronousBusRequest(prSdInfo->hDevice, SD_CMD_IO_RW_EXTENDED, argument, SD_WRITE, ResponseR5, &response, u4BlockNum,	/*              */
						 prSdInfo->sdHostBlockCap.WriteBlockSize,	/*            */
						 pucBuf, 0);
#endif
