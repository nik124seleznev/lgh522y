...
void platformBusInit(IN P_GLUE_INFO_T prGlueInfo)
{
	...
	/*            */
	NDIS_PHYSICAL_ADDRESS CLKMGR_Base = { CLKMGR_BASE_ADD };	/*                  */
	NDIS_PHYSICAL_ADDRESS SSP_Base = { SSP_BASE_ADD };	/*     */
	NDIS_PHYSICAL_ADDRESS GPIO_Base = { GPIO_BASE_ADD };	/*      */
	...
	UINT_32 GpioAltFnIntPinList[] = { 1, GPIO_INTR };	/*           */
	...
	prHifInfo->u4InterruptVector = u4Irq;	/*                  */
	prHifInfo->u4InterruptLevel = u4Irq;	/*                  */
	...
	if (pVMem) {
		...
		GpioSetFallingEdgeDetectEnable((P_GPIO_REG) pVMem, GpioAltFnIntPinList);	/*           */
	}
	...
	if (pVSsp) {
		...
		pVSsp->sscr1 = 0;
		pVSsp->sscr1 = SSCR1_SPH | SSCR1_SPO | SSCR1_TTE;	/*          */
		pVSsp->sscr1 |= (3 << 6) | (0 << 10);	/*          */
...
{
	IO_GPDR0 = pVMem->GPDR0 & (BITS(23, 26) | IO_MASK_INTR);	/*                  */
	Fun_GAFR0_U = pVMem->GAFR0_U & (BITS(14, 21) | FUN_MASK_INTR);	/*                  */
}				/*                  */
...
VOID GPIOConfigRestore(VOID)
{
	...
	tempA = IO_GPDR0 & (BITS(23, 26) | IO_MASK_INTR);	/*                  */
	tempB = pVMem->GPDR0 & ~(BITS(23, 26) | IO_MASK_INTR);
	pVMem->GPDR0 = tempA | tempB;

	tempA = Fun_GAFR0_U & (BITS(14, 21) | FUN_MASK_INTR);	/*                  */
