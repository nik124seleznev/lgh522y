     ...
     */
			...
			if (prAdapter->rWifiVar.prP2pFsmInfo->eCurrentState == P2P_STATE_LISTEN) {
				...
				if (prIeSupportedRate || prIeExtSupportedRate) {
					...
					rateGetRateSetFromIEs(prIeSupportedRate, prIeExtSupportedRate, &u2OperationalRateSet, &u2BSSBasicRateSet,	/*                      */
...
VOID ibssInitForAdHoc(IN P_ADAPTER_T prAdapter, IN P_BSS_INFO_T prBssInfo)
{
	...
	/*                                         */
	if (rNonHTPhyAttributes[prBssInfo->ucNonHTBasicPhyType].fgIsShortPreambleOptionImplemented && ((prAdapter->rWifiVar.ePreambleType == PREAMBLE_TYPE_SHORT) ||	/*                                      */
