...
static struct cfg80211_ops mtk_p2p_config_ops = {
	...
	/*       */
	.change_virtual_intf = mtk_p2p_cfg80211_change_iface,	/*      */
				  ...
				  IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	if (prData->flags == P2P_STOP_DISCOVERY) {
		...
		if (prReqDeviceType->scan_type == P2P_LISTEN) {
			...
			prDiscoverMsg->u4DevDiscoverTime = 0;	/*           */
			   ...
			   IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSetNoaParam, prNoaParam,	/*                      */
...
/*                                                                          */
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidSetOppPsParam, prOppPsParam,	/*                      */
