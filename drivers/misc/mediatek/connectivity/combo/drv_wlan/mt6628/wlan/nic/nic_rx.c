...
#if CFG_RX_PKTS_DUMP
	{
		...
		if (prAdapter->rRxCtrl.u4RxPktsDumpTypeMask & BIT(HIF_RX_PKT_TYPE_MANAGEMENT)) {
			if (u2TxFrameCtrl == MAC_FRAME_BEACON ||
				...
				DBGLOG(SW4, INFO, ("QM RX MGT: net %lu sta idx %u wlan idx %u ssn %lu ptype %u subtype %u 11 %u\n", HIF_RX_HDR_GET_NETWORK_IDX(prHifRxHdr), prHifRxHdr->ucStaRecIdx, prSwRfb->ucWlanIdx, HIF_RX_HDR_GET_SN(prHifRxHdr),	/*                         */
	...
	case CATEGORY_SA_QUERT_ACTION:
		{
			...
			if ((HIF_RX_HDR_GET_NETWORK_IDX(prHifRxHdr) == NETWORK_TYPE_AIS_INDEX) && prAdapter->rWifiVar.rAisSpecificBssInfo.fgMgmtProtection	/*         */
			    ) {
