			...
			IN UINT_8 aucDeviceID[], IN PP_BSS_DESC_T pprBssDesc)
{
	...
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
		...
		prTargetP2pDesc = scanFindP2pDeviceDesc(prAdapter,
							prBssDesc, aucDeviceID, TRUE, FALSE);
	...
	do {
		...
		if ((fgAddIfNoFound) && (prTargetDesc == NULL)) {
			...
			if (prTargetDesc) {
				...
				LINK_INSERT_TAIL(&prBssDesc->rP2pDeviceList,
						 &prTargetDesc->rLinkEntry);
...
BOOLEAN scanUpdateP2pDeviceDesc(IN P_ADAPTER_T prAdapter, IN P_BSS_DESC_T prBssDesc)
{
	...
	p2pGetP2PAttriList(prAdapter, prBssDesc->aucIEBuf, prBssDesc->u2IELength,
			   (PPUINT_8) & prP2pAttribute, &u2AttributeLen);

	while (u2AttributeLen >= P2P_ATTRI_HDR_LEN) {
		switch (prP2pAttribute->ucId) {
		case P2P_ATTRI_ID_P2P_CAPABILITY:	/*                        */
			{
				P_P2P_ATTRI_CAPABILITY_T prP2pAttriCapability =
				    (P_P2P_ATTRI_CAPABILITY_T) NULL;

				prP2pAttriCapability = (P_P2P_ATTRI_CAPABILITY_T) prP2pAttribute;
				ASSERT(prP2pAttriCapability->u2Length == 2);

				prP2pDesc->ucDeviceCapabilityBitmap =
				    prP2pAttriCapability->ucDeviceCap;
				prP2pDesc->ucGroupCapabilityBitmap =
				    prP2pAttriCapability->ucGroupCap;
			}
			break;
		case P2P_ATTRI_ID_P2P_DEV_ID:	/*        */
			{
				...
				kalMemCopy(prP2pDesc->aucDeviceAddr, prP2pAttriDevID->aucDevAddr,
					   MAC_ADDR_LEN);
			}
			break;
		case P2P_ATTRI_ID_P2P_DEV_INFO:	/*                */
			{
				P_P2P_ATTRI_DEV_INFO_T prP2pAttriDevInfo =
				    (P_P2P_ATTRI_DEV_INFO_T) NULL;
				...
				kalMemCopy(prP2pDesc->aucDeviceAddr, prP2pAttriDevInfo->aucDevAddr,
					   MAC_ADDR_LEN);

				WLAN_GET_FIELD_BE16(&prP2pAttriDevInfo->u2ConfigMethodsBE,
						    &prP2pDesc->u2ConfigMethod);

				prP2pDevType = &prP2pDesc->rPriDevType;
				WLAN_GET_FIELD_BE16(&prP2pAttriDevInfo->rPrimaryDevTypeBE.
						    u2CategoryId, &prP2pDevType->u2CategoryID);
				WLAN_GET_FIELD_BE16(&prP2pAttriDevInfo->rPrimaryDevTypeBE.
						    u2SubCategoryId,
						    &prP2pDevType->u2SubCategoryID);

				ASSERT(prP2pAttriDevInfo->ucNumOfSecondaryDevType <= P2P_GC_MAX_CACHED_SEC_DEV_TYPE_COUNT);	/*                                                      */
				prP2pDesc->ucSecDevTypeNum = 0;
				for (u4Idx = 0; u4Idx < prP2pAttriDevInfo->ucNumOfSecondaryDevType;
				     u4Idx++) {
					if (u4Idx < P2P_GC_MAX_CACHED_SEC_DEV_TYPE_COUNT) {
						...
						WLAN_GET_FIELD_BE16(&prP2pAttriDevInfo->
								    arSecondaryDevTypeListBE[u4Idx].
								    u2CategoryId,
				...
				prP2pDevName =
				    (P_DEVICE_NAME_TLV_T) ((PUINT_8) prP2pAttriDevInfo->
							   arSecondaryDevTypeListBE +
		...
		prP2pAttribute =
		    (P_P2P_ATTRIBUTE_T) ((UINT_32) prP2pAttribute +
					 (prP2pAttribute->u2Length + P2P_ATTRI_HDR_LEN));
	...
	if (prP2pAttriGroupInfo != NULL) {
		...
		while (u2AttributeLen > 0) {
			prP2pDesc =
			    scanFindP2pDeviceDesc(prAdapter, prBssDesc,
						  prClientInfoDesc->aucDevAddr, TRUE, TRUE);
			...
			WLAN_GET_FIELD_BE16(&prClientInfoDesc->u2ConfigMethodsBE,
					    &prP2pDesc->u2ConfigMethod);
			...
			ASSERT(prClientInfoDesc->ucNumOfSecondaryDevType <=
			...
			u2AttributeLen -=
			    (prClientInfoDesc->ucLength + P2P_CLIENT_INFO_DESC_HDR_LEN);
			prClientInfoDesc =
			    ...
			    IN P_BSS_DESC_T prBssDesc, IN P_SW_RFB_T prSwRfb)
{
	...
	LINK_FOR_EACH(prLinkEntry, &prBssDesc->rP2pDeviceList) {
		...
		kalMemCopy(&rEventDevInfo.rPriDevType,
			   &prTargetDesc->rPriDevType, sizeof(P2P_DEVICE_TYPE_T));
		...
		kalMemCopy(rEventDevInfo.aucName,
			   prTargetDesc->aucName, prTargetDesc->u2NameLength);
		...
		case P2P_ATTRI_ID_P2P_CAPABILITY:
			{
				...
				prP2pAttriCapability = (P_P2P_ATTRI_CAPABILITY_T) prP2pAttribute;
				ASSERT(prP2pAttriCapability->u2Length == 2);
				rEventDevInfo.ucDeviceCapabilityBitmap =
				    prP2pAttriCapability->ucDeviceCap;
				rEventDevInfo.ucGroupCapabilityBitmap =
				    prP2pAttriCapability->ucGroupCap;
			}
			break;
		case P2P_ATTRI_ID_P2P_DEV_ID:
			{
				...
				kalMemCopy(rEventDevInfo.aucCommunicateAddr,
					   prP2pAttriDevID->aucDevAddr, MAC_ADDR_LEN);
			}
			break;
		case P2P_ATTRI_ID_P2P_DEV_INFO:
			{
				...
				prP2pDevName =
				    (P_DEVICE_NAME_TLV_T) (prP2pAttriDevInfo->
							   arSecondaryDevTypeListBE +
							   (u4Idx * sizeof(DEVICE_TYPE_T)));
				ASSERT(prP2pDevName->u2Id == 0x1011);
				ASSERT(prP2pDevName->u2Length <= 32);	/*                                                            */
				kalMemCopy(rEventDevInfo.aucName, prP2pDevName->aucName,
					   prP2pDevName->u2Length);
			}
			break;
		case P2P_ATTRI_ID_P2P_GROUP_INFO:
			prP2pAttriGroupInfo = (P_P2P_ATTRI_GROUP_INFO_T) prP2pAttribute;
	...
	if (prP2pAttriGroupInfo != NULL) {
		...
		while (u2AttributeLen > 0) {
			...
			ASSERT(u2AttributeLen >= 25);
			rEventDevInfo.fgDevInfoValid = TRUE;
			kalMemCopy(rEventDevInfo.aucCommunicateAddr, prClientInfoDesc->aucIfAddr,
				   MAC_ADDR_LEN);
			...
			prP2pDevType = &rEventDevInfo.rPriDevType;
			prP2pDevType->u2CategoryID =
			    prClientInfoDesc->rPrimaryDevTypeBE.u2CategoryId;
			prP2pDevType->u2SubCategoryID =
			    prClientInfoDesc->rPrimaryDevTypeBE.u2SubCategoryId;
			...
			for (u4Idx = 0; u4Idx < prClientInfoDesc->ucNumOfSecondaryDevType; u4Idx++) {
				...
				prP2pDevType->u2SubCategoryID =
				    prClientInfoDesc->arSecondaryDevTypeListBE[u4Idx].
				    u2SubCategoryId;
			}
			...
			u2AttributeLen -= prP2pAttriGroupInfo->u2Length;
			prP2pAttriGroupInfo =
			    prP2pAttriGroupInfo + prP2pAttriGroupInfo->u2Length + 1;
		}
