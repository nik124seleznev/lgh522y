...
static cmdqSecContextHandle gCmdqSecContextHandle = NULL; /*                             */
/*          */
			...
			const int32_t iwcCommand, TaskStruct *pTask, void *data)
{
	...
	if (CMD_CMDQ_TL_CANCEL_TASK == iwcCommand) {
		...
		if (pCancelResult) {
			...
			pCancelResult->errInstr[0]= pIwc->cancelTask.errInstr[0]; /*      */
			pCancelResult->errInstr[1] = pIwc->cancelTask.errInstr[1]; /*      */
